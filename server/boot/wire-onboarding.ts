...
import { unitApexFromChain } from "../domains/onboarding/admission-policy.ts";
...
import { makeClusterKubeResolver } from "../domains/onboarding/cluster-kube.ts";
...
import { Registry, clusterStageFromMarkings } from "../domains/onboarding/registry.ts";
import { TenantRegistry, CATALOG_CHART_BRANCH } from "../domains/onboarding/tenant-registry.ts";
import type { ClusterStageResolver } from "../domains/onboarding/registry.ts";
import { makeOnboardDef, type OnboardPorts } from "../domains/onboarding/onboard.run.ts";
import { makeOffboardDef } from "../domains/onboarding/offboard.run.ts";
import { makePurgeDef } from "../domains/onboarding/purge.run.ts";
import { makeAdoptConsumerDef } from "../domains/onboarding/adopt-consumer.run.ts";
import { makeSuspendDef, makeResumeDef } from "../domains/onboarding/suspend-resume.run.ts";
import { makeRestartWorkloadsDef, makeTenantRestartWorkloadsDef } from "../domains/onboarding/restart-workloads.run.ts";
...
import { makeOffboardTenantDef } from "../domains/onboarding/offboard-tenant.run.ts";
import { makeTenantPurgeDef } from "../domains/onboarding/tenant-purge.run.ts";
import { HttpPublicProbe } from "../adapters/http-probe/http-probe.ts";
import type { RelocationPorts } from "../domains/onboarding/relocation.ts";
import type { ConsumerRelocationPorts } from "../domains/onboarding/relocation-world-consumer.ts";
import type { TenantRelocationPorts } from "../domains/onboarding/relocation-world-tenant.ts";
import { makeBackupDef, makeTenantBackupDef } from "../domains/onboarding/backup.run.ts";
import { makeRestoreDef, makeTenantRestoreDef } from "../domains/onboarding/restore.run.ts";
import { makeMigrateDef, makeTenantMigrateDef } from "../domains/onboarding/migrate.run.ts";
import { errValidation } from "../kernel/errors.ts";

// The onboarding composition — the only place the real
// onboarding adapters are constructed and handed to the Run families. Kept out of wire.ts to keep that
// file focused. Two INDEPENDENT families live here:
//
//  - CONSUMER onboarding: the Tekton gate-runner + platform/kube adapters. Goes live ONLY when BOTH
//    the gate-runner config (ONBOARD_GATE_CONTROLLER_ADDR) and the platform repo (github) are
//    configured — a partial config is a 501, never a half-wired feature.
//  - TENANT (multi-app) onboarding: a SECOND GitPlatformRepo bound to catalog + the
//    controller-side HelmRenderer (tenant charts are trusted first-party, validated controller-side —
...
//
// Kube access never gates either family: the clients reach the Controller's OWN cluster in-cluster
...
//
// buildOnboarding returns both families' defs merged (buildRegistry consumes them together) plus the
...
// (job-budget-ms param); the controller-side validation poll is bounded a margin ABOVE it, so on
...
// Where the Deployment mounts the controller-registry-pull dockerconfigjson (hostyour-cloud
// apps/controller/templates/deployment.yaml, volume `registry-pull`) — the SAME pull credential the
...
const REGISTRY_PULL_DOCKERCONFIG_PATH = "/etc/controller/registry-pull/.dockerconfigjson";
...
export interface OnboardingWiring {
  ...
  /** The shared activation client (ONE HttpActivator for the whole controller), threaded to
  ...
  activator?: Activator;
  /** The TENANT family's catalog pointer registry, threaded to registerTenantRoutes so the
   *  operator-triggered orphan scan (GET /api/tenants/orphans) can diff the LIVE pointers against the
   *  inventory. The SAME TenantRegistry the tenant runs commit through — one reader of tenants/**.
   *  Undefined when tenant onboarding is not configured; the scan route then degrades to an empty
   *  result with a reason. */
  tenantRegistry?: TenantRegistry;
  /** The CONSUMER family's registration registry, threaded to registerConsumerRoutes so the
   *  operator-triggered DETECTED scan (GET /api/consumers/detected) can diff the
   *  LIVE registrations/** against the inventory — the consumer twin of tenantRegistry above, and the
   *  SAME Registry the consumer runs commit through. Undefined when consumer onboarding is not
  ...
  /** The writer of the PLATFORM repo (hostyour-cloud), handed to buildRegistry so deploy-slave can mark
...
interface Family {
  ...
  catalogRepoUrl?: string;
  /** Present only for the tenant family — the pointer registry its orphan-scan read route diffs
   *  against the inventory. Undefined when the family is not configured. */
  tenantRegistry?: TenantRegistry;
  /** Present only for the consumer family — the registration registry its detected-scan read route
...
export function buildOnboarding(config: Config, store: CredentialStore, db: Db, logger: Logger): OnboardingWiring {
  // ONE activation client for the whole controller — a plain fetch to a consumer's / tenant's OWN public
  ...
  // the ports are built — not looked up per run. The Controller runs ON the cluster holding the master
  ...
  // The ONE writer of the platform repo: the consumer pointer registry commits through it, and
  ...
  // markings (clusters/active/<fqdn>.yaml on the platform repo). The tenant registry's own repo is
  ...
  // (global.unitApex) here, and handed over whole for the tenant planners, which derive the registry
  ...
  const consumer = buildConsumerOnboarding(config, store, db, activator, logger, platformRepo, clusterStage, dns, relocation, tenant.tenantRegistry, seeder);
  // The sanctioned type-erasure (registry.ts): each typed RunDefinition<P> is stored executor-facing
  // as AnyRunDefinition; the executor parses params via paramsSchema before plan()/steps(). Both
  // families share one flat defs[] — the run kinds are disjoint, so buildRegistry keys them apart.
  ...
  return {
    ...
    ...(consumer.registry ? { registry: consumer.registry } : {}),
    ...
    ...(tenant.tenantRegistry ? { tenantRegistry: tenant.tenantRegistry } : {}),
...
function buildConsumerOnboarding(
  ...
  /** The SAME seeder the tenant family writes through — built once in buildOnboarding, because there is
   *  one Vault and one Controller identity. */
  seeder: VaultSeeder,
): Family {
  ...
  const registry = new Registry(platformRepo, clusterStage);
  ...
  // locked-down `gate-runner` namespace on the control cluster (over the Controller pod's own SA, or
  ...
  // The Controller's one writing kube client: the per-consumer isolation AppProject on the master's
  ...
  // stateless instance serves every consumer. The HMAC secret is fed to the controller as env
  ...
  // the per-unit controller-read grant provision-build-rbac wrote. A pure watch; the run itself is
  ...
  const onboardPorts: OnboardPorts = {
    ...
    runner,
    registry,
    // G23's tenant-subdomain clause. The tenant family owns the catalog pointer registry, so a
    // controller wired for consumers but not for tenants cannot answer the question — and answering
    ...
    tenantSubdomains: () => {
      if (!tenantRegistry) {
        throw errValidation(
          "onboarding a consumer requires the tenant pointer registry (config.catalog) to check the unit name against the tenants' subdomains — a consumer named after one serves the host that tenant's example-auth scopes its session cookies to",
        );
      }
      return tenantRegistry.listTenantSubdomains();
    },
    seeder,
    resolver,
    ...
    platformRepoURL: `https://github.com/${config.github.owner}/${config.github.repo}.git`,
    // The gate-runner's own sandbox self-probe is the primary lock; a controller-side probe of the
    // must-fail targets before each job is a follow-up, so this attestation is asserted for now.
    attestListening: true,
    // The controller-side bound of the validation poll — a margin above the sandbox job budget
    ...
    // with the tenant family (buildOnboarding constructs the one instance).
    activator,
    ...
    // so the Controller writes them. UNCONDITIONALLY needed — absent ⇒ the step fails loud.
    buildRbac,
  };
  const lifecyclePorts: LifecyclePorts = { registry, resolver, argoWatchTimeoutMs: ARGO_WATCH_TIMEOUT_MS };
  ...
  const defs: AnyRunDefinition[] = [
    ...
    // the same narrow lifecycle port set (registry read + resolver for attest-target/attest-live).
  ...
  // access at request time (the same resolver the runs already resolve through); the registry rides
  // out so the detected scan (GET /api/consumers/detected) diffs the very pointers the runs commit.
  return { defs, enabled: true, resolver, registry };
}

// ---- Tenant (multi-app) onboarding: catalog + the controller-side HelmRenderer ----
function buildTenantOnboarding(
  ...
  // repo at a ref for controller-side validation, and the platform repo pushes tenant pointers. An
  ...
  const helm = new HelmCliRenderer(); // trusted first-party charts render controller-side (no sandbox)
  ...
  const registry = new TenantRegistry(deployRepo, clusterStage);
  const argo = new KubeMasterArgoReader(masterKubeInput(config));
  const cluster = new KubeClusterReader(masterKubeInput(config));
  const projects = new KubeMasterProjectWriter(masterKubeInput(config));
  ...
  // The ensure-images gate: the registry probe reads the mounted controller-registry-pull
  ...
  const buildRbac = new KubeBuildRbacWriter(masterKubeInput(config));
  const registrations = new Registry(platformRepo, clusterStage);
  ...
  const onboardPorts: TenantOnboardPorts = {
    ...
    // family uses (one instance, from buildOnboarding). Used only when the operator supplies an admin email.
    activator,
  ...
  // clone/render, so they take the narrower lifecycle port set (registry + resolver).
  ...
  // The tenant relocation ports: the lifecycle set (registry/resolver/dns/argo-sync/apex) plus the
  ...
  // appCatalog + resolver + the repo URL + the registry ride out so registerTenantRoutes can serve GET
  ...
  // through the very registry the runs commit pointers with — all the same instances (and the same one
  // repoURL the appsets are rendered from) the runs use, never a second one.
  return { defs, enabled: true, resolver, catalogRepoUrl: repoURL, appCatalog, tenantRegistry: registry };
}
