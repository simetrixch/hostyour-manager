// The RELEASE surface — which version each of an installation's platform apps runs. The platform
// could deploy a version and could not show one anywhere; this is where that question is answered.
//
// An INSTALLATION is a registered cluster. Its FQDN is its install branch (clusters.domain), and
// that branch carries the answer: the `builds[]` pins of
// clusters/inventories/<app>/values-<stage>.yaml ON THAT BRANCH, at the cluster's OWN stage. An
// install branch stands on the state the cluster actually runs, which can be older than the trunk,
// so the trunk's pins would answer a different question.
//
...
//
// DEGRADE LOUD. A failed pin search names itself rather than shrinking into an empty list.
...
export interface ReleasesApiDeps {
   ...
   *  git port and the domain that owns the search. Absent ⇒ not configured; the rows are still
...
export async function buildReleasesView(deps: ReleasesApiDeps): Promise<ReleasesView> {
  // The rows are answered whatever the app search does — those come from the database, and half an
  // answer beats none.
  const rows = installationRows(deps.db);
