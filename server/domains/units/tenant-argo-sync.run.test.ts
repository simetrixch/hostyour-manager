...
async function seededRegistrations(): Promise<TenantRegistrations> {
  ...
  const registration: TenantRegistration = {
    ...
    members: testMembers(APPS), identityProvider: "auth", routing: "host", ownDomain: "", ownDomainRedirects: [], approvedTags: {}, apps: APPS.map((a) => ({ name: a.name, seedReference: false, seedDemo: false, selections: {} })),
