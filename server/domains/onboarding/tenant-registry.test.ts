...
import { TenantRegistry, tenantRegistrationWrite } from "./tenant-registry.ts";
...
describe("TenantRegistry", () => {
  ...
  it("commitTenant refuses a registration whose cluster is marked for a DIFFERENT stage, and the error names both sides", async () => {
    const reg = new TenantRegistry(new FakePlatformRepo(), marked({ s1: "dev" }));
...
describe("the pointer scan reports what it could NOT read", () => {
  ...
  /** A registry whose fake catalog carries GUID as a clean registration plus one seeded raw file. */
  async function withSeeded(path: string, content: string): Promise<TenantRegistry> {
    const repo = new FakePlatformRepo();
    const reg = new TenantRegistry(repo, CLUSTERS);
    await reg.commitTenant({ stage: "dev", guid: GUID, registration: registration(), runId: "run_1" });
