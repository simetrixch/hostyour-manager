...
// else: the Controller clones the (possibly private) consumer repo ITSELF to resolve the ref (
...
// controller-side gates, and returns the verdict.
//
...
import { composeReport, gateG16, gateG17, gateG18, gateG19, gateG23, type ForeignBuild, type ForeignFqdn } from "./gates/compose.ts";
...
export interface OnboardRequest {
  ...
  ref: string;
  consumerName: string;
  repoCredentialId?: string; // a Controller-side credential id for a private repo
}
...
/** Reads the build names the OTHER units have attested. Registry implements it over
 ...
 *  onboarded (a re-onboard must not collide with itself). Registry implements it over every
...
/** Every subdomain a TENANT stands at, over every stage — TenantRegistry.listTenantSubdomains over
 *  the catalog pointers. It is a second repo and therefore a dep of its own, not a method on
 *  the consumer registry above. G23 holds the candidate unit name against it: the two spaces compose
...
export interface ValidateDeps {
  ...
  registry: AttestedBuildReader & AttestedFqdnReader;
  ...
  signal: AbortSignal;
  /** The Controller's confirmed-listening attestation for the must-fail probe targets. */
...
export interface ValidationOutcome {
  verdict: "pass" | "fail";
  resolvedSha: string;
  report: GateReport; // the composed report (runner sandbox gates + controller-side gates)
 ...
 *  gate PipelineRun clones the repo ITSELF at that revision; the Controller passes the private-repo
...
export async function validateOnboard(req: OnboardRequest, target: OnboardTarget, deps: ValidateDeps): Promise<ValidationOutcome> {
  ...
  try {
    ...
    const foreignBuilds = await deps.registry.listAttestedBuildNames(req.consumerName);
    ...
    const foreignFqdns = declaredFqdn !== null ? await deps.registry.listAttestedFqdns({ unit: req.consumerName, stage: target.stage }) : [];

    const controllerGates: GateResult[] = [
      gateG17({ ok: true, detail: `cloned ${req.repoURL} at ${cloned.resolvedSha}` }),
      ...
      gateG23({ unitName: req.consumerName, tenantSubdomains }),
      gateG16({ unitName: req.consumerName, buildNames: declaredBuilds, foreignBuilds }),
      gateG18({ declaredBuilds, chart }),
      gateG19({ unitName: req.consumerName, fqdn: declaredFqdn, unitApex, clusterDomain, foreignFqdns }),
    ];
    for (const g of controllerGates) deps.log(`${g.id} ${g.status} — ${g.detail}`);

    const report = composeReport(runnerReport, controllerGates);
