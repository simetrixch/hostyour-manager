...
// shared/api-types.ts and used here — the same rule tenant-registry.ts follows for
 ...
 *  `..` traversal, else it is a programming error (INTERNAL), never a commit. Each registry binds its
 *  own guard to its namespace regex (the consumer `guard` below; the tenant registry supplies its own
 ...
 *  registration writers take this rather than a PlatformRepo, because the tenant registry's own repo is
 ...
 *  registry's schemaWhy so a broken registration reads identically across the two formats. */
...
export class Registry {
   ...
   *  once when the repo port was built. Exposed because a run that commits through this registry
   ...
   *  registry repository, and this tree is where every claim on one stands.
   *
   ...
   *  AND every one the scan had to skip — the consumer twin of the tenant registry's listTenantPointers,
