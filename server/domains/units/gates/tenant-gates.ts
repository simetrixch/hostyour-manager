...
//                   Manager render bypasses the sandbox, so this faithfully replicates the sandbox
...
// by the Manager, so a chart emitting one would be a second writer of the same object — it is
...
function evaluate(kind: string, name: string, raw: Record<string, unknown> | null, where: string, memberNs: string): Violation | null {
  ...
  if (ARGO_KINDS.has(kind)) {
    return {
      found: `${where} has kind "${kind}", named "${name}".`,
      reason: `kind "${kind}" is an argoproj.io control resource; a tenant chart may not mint its own ArgoCD Application/ApplicationSet/AppProject — the Manager generates the fan-out.`,
      fieldPath: "kind", value: kind,
    };
  }
...
export function gatePinnedSha(resolvedSha: string): GateResult {
  ...
  return {
    ...
    evidence: [{ source: "manager", value: resolvedSha.slice(0, 256) }],
  };
}
 ...
 *  every HARD gate passed — the manager authors ALL the gates, so there is no separate runner/sandbox
