...
function ports(reg: TenantRegistry, over: Partial<TenantLifecyclePorts> & FakeKube = {}): TenantLifecyclePorts {
...
describe("tenant-suspend run", () => {
  ...
  it("plans with tenant targetKind, the five steps, and the repo-qualified git-branch + master-kube locks", async () => {
    seedTenant();
    const plan = await makeSuspendTenantDef(ports(new TenantRegistry(new FakePlatformRepo(), CLUSTERS))).plan({ tenantId: "tnt_1" }, { db: db.db });
...
describe("remove-app run", () => {
  ...
  it("plans with the four steps + remove-app kind", async () => {
    seedTenant({ apps: ["erp", "web"] });
    const plan = await makeRemoveAppDef(ports(new TenantRegistry(new FakePlatformRepo(), CLUSTERS))).plan({ tenantId: "tnt_1", app: "web" }, { db: db.db });
...
describe("tenant-offboard run", () => {
  it("plans with tenant targetKind, the five steps, and the tenant locks", async () => {
    seedTenant();
    const def = makeOffboardTenantDef(ports(new TenantRegistry(new FakePlatformRepo(), CLUSTERS)));
  ...
  it("record-offboard REFUSES TO DOWNGRADE a tenant a purge already settled", async () => {
    ...
    const rec = makeOffboardTenantDef(ports(new TenantRegistry(new FakePlatformRepo(), CLUSTERS))).steps({ tenantId: "tnt_1" }).find((s) => s.name === "record-offboard")!;
