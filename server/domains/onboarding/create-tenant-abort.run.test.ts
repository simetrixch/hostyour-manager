...
function fakeTenantSeeder(): VaultSeeder {
  ...
  // The sanctioned type-erasure at the registry boundary, exactly as wire-onboarding.ts does it.
  const def = makeCreateTenantDef(ports) as unknown as AnyRunDefinition;
  const executor = new Executor({ db: db.db, creds: store, bus, logger, registry: buildRegistry({ db: db.db }, [def]), sshFactory: noSsh, actor: () => "op_system" });
  ...
  return { app, cookie: await session.mint({ sub: "op_test", groups: ["admins"], via: "oidc" }), executor, registry, argo, projects, cluster };
}
...
describe("aborting a create-tenant run whose tenant went LIVE", () => {
  it("is REFUSED, and nothing of the deployed tenant is touched", async () => {
    ...
    expect(await h.registry.readTenant("prod", params.guid)).not.toBeNull();
  ...
  it("still rolls back the run it is FOR: an unfinished tenant is torn down as before", async () => {
    ...
    expect(await h.registry.readTenant("prod", params.guid)).toBeNull(); // pointer git-rm'd
    for (const member of [...TEST_MEMBERS, ...params.apps.map((a) => a.name)]) {
