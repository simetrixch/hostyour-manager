...
// therefore whether a run kind that would take it off its cluster may proceed at all. It is stated here
...
//     both run kinds are the remedy for (the purge from the Tenants list, the abort from the run screen).
...
//     purge is the only run kind that reaps them. The Mongo databases do NOT outlive it: the prune deletes
...
//     and serving, and tenant-offboard is the removal run kind for it (the tenant can be re-onboarded
...
//     with no removal run kind at all — tenant-offboard cannot settle it while its fan-out refuses to prune,
...
/** How ONE caller names ITSELF inside the refusal. The RULE is shared; the WORDS are the run kind's own,
 *  because "what would have happened to this tenant" is exactly what the operator needs to read and the
 *  two run kinds do very different things to it. Each run kind builds this once and hands the same value to both
...
export interface TenantLiveRefusal {
   ...
   *  that says the manager could not establish the tenant is already un-deployed. */
  subject: string;
 ...
 *  pointer is gone. Throws VALIDATION on a live tenant, NOT_CONFIGURED when no registrations is wired and the
...
export async function assertTenantNotLive(
  db: Db,
  registrations: TenantRegistrations | undefined,
  ...
  if (!registrations) {
    throw errNotConfigured(`${named} Its GitOps pointer cannot be read on this manager, so ${refusal.subject} cannot establish that the tenant is already un-deployed`);
  }
  if ((await registrations.scanTenant(t.stage, t.guid)).status !== "absent") {
