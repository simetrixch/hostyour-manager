...
async function seedRegistration(
  ...
  await registrations.commitRegistration({
    ...
    builds: [],
    deploy: { stage: opts.stage ?? "prod", host: name, chartPath: "deploy/chart", cluster: opts.cluster, databases: [], keyPatterns: [], channelPatterns: [], services: [], size: "small", mongodb: "shared", quota: seedQuota("small") },
    runId: `run_${name}`,
  });
}
...
function seedApp(name: string, over: { status?: "active" | "suspended" | "offboarded" } = {}): void {
  db.db
    .insert(apps)
    .values({ id: `app_${name}`, clusterId: "cls_1", name, stage: "prod", host: name, repoUrl: `https://github.com/x/${name}.git`, chartPath: "deploy/chart", provenance: "manager", status: over.status ?? "active" })
    .run();
}
...
describe("scanDetectedConsumers (the registration-vs-inventory diff)", () => {
  ...
  it("propagates a registrations failure instead of answering an empty list", async () => {
    ...
    await expect(scanDetectedConsumers({ db: db.db, registrations })).rejects.toThrow("install branch unreachable");
  });
});
...
describe("scanClusterOrphanConsumers (the cluster-vs-both-books diff)", () => {
  ...
  it("carries a cluster it could NOT read out as unscanned, and keeps scanning the others", async () => {
    ...
    const found = await scanClusterOrphanConsumers({ db: db.db, registrations, resolver });
    expect(found.clusterOrphans.map((o) => o.name)).toEqual(["ghost"]);
