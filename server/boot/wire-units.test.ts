...
// actually produces, so the declaration the registrations.total boot check reasons about stays the wiring's
...
describe("buildUnits enable gates (wire-units.ts)", () => {
  ...
  it("enables BOTH families WITHOUT a kubeconfig — in-cluster (pod SA) is the default kube access", () => {
    ...
    // The tenant read routes' collaborators ride out with the family — the pointer registrations is what the
  ...
  it("still honors a set KUBECONFIG_PATH as the explicit file override", () => {
    ...
    expect(wiring.enabled).toBe(true);
    expect(wiring.tenantEnabled).toBe(true);
    expect(wiring.defs).toHaveLength(30); // 12 consumer (incl. purge, adopt-consumer, restart-workloads, set-size, set-secrets, backup/restore/migrate) + 18 tenant (those 12, plus check-tenants, tenant-apps-repo, tenant-apps-repo-purge, tenant-set-routing, tenant-set-own-domain and tenant-refresh-members)
  });
  ...
  it("BOTH families stay off without MASTER_FQDN — nothing may write the books without knowing which branch they are", () => {
    ...
    // configured value is the only statement of it: buildUnits runs BEFORE seedMaster writes
  ...
  it("consumer stays off without the gate-runner config; the tenant family is independent", () => {
    const wiring = buildUnits(...setup({ ONBOARD_GATE_MANAGER_ADDR: undefined }));
    expect(wiring.enabled).toBe(false);
    expect(wiring.tenantEnabled).toBe(true);
    expect(wiring.defs.map((d) => d.kind).sort()).toEqual([...TENANT_KINDS].sort());
    // No consumer family ⇒ no Registrations, so the detected-scan route degrades to its reason
    // instead of scanning through a half-wired repo (the tenantRegistrations twin below).
  ...
  it("tenant stays off without CATALOG_REPO; the consumer family is independent", () => {
    ...
    // No family ⇒ no registrations, so the orphan-scan route degrades to its reason instead of scanning
    // through a half-wired repo.
    expect(wiring.tenantRegistrations).toBeUndefined();
  });
