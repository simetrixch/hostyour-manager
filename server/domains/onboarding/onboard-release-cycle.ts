...
//
// The controller has no task in the release cycle — it only triggers and reads. Everything the
...
export function triggerReleaseStep(ports: OnboardPorts, p: OnboardParams, runtime: ReleaseCycleRuntime): Step {
  return {
    ...
    run: async (ctx) => {
      if (!ports.github) {
        throw errValidation(`onboard "${p.consumerName}" requires the GitHub client to trigger the release workflow but none is wired on this controller — the release cycle cannot start`);
      }
      ...
      ctx.log("meta", `release workflow dispatched on ${owner}/${repo} — ${RELEASE_WORKFLOW_FILE} with version=${p.version} channel=${p.channel} stage=${p.stage}; the cycle now runs outside the controller`);
    },
  };
}
...
export function watchReleaseWorkflowStep(ports: OnboardPorts, p: OnboardParams, runtime: ReleaseCycleRuntime): Step {
  return {
    ...
    run: async (ctx) => {
      if (!ports.github) {
        throw errValidation(`onboard "${p.consumerName}" requires the GitHub client to watch the release workflow but none is wired on this controller`);
      }
      const { owner, repo } = parseGitHubOwnerRepo(p.repoURL);
 ...
 *  kept in-run for the log — the controller never composes it. */
export function watchReleaseBuildStep(ports: OnboardPorts, p: OnboardParams, runtime: ReleaseCycleRuntime): Step {
  return {
    ...
    run: async (ctx) => {
      if (!ports.buildPlane) {
        throw errValidation(`onboard "${p.consumerName}" requires the build-plane client to watch the release run but none is wired on this controller`);
      }
