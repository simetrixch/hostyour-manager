...
describe("offboard remove-registration on resume", () => {
  it("a second run after its git commit skips instead of wedging", async () => {
    ...
    db.db.insert(apps).values({ id: "app_1", clusterId: "cls_1", name: "acme", stage: "prod", repoUrl: "https://github.com/x/acme.git", chartPath: "deploy/chart", provenance: "manager", status: "active", repoCredentialId: null }).run();
