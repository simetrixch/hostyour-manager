...
async function seedRegistration(reg: Registry, over: { suspended?: boolean } = {}): Promise<void> {
...
function ports(reg: Registry, status: ArgoAppStatus): LifecyclePorts {
  return {
    registry: reg,
...
function seedApp(status: "active" | "suspended"): void {
  ...
  db.db.insert(apps).values({ id: "app_1", clusterId: "cls_1", name: "acme", stage: "prod", repoUrl: "https://github.com/x/acme.git", chartPath: "deploy/chart", provenance: "controller", status }).run();
}
