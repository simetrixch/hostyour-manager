...
// The AppProject, the namespace and the Vault entries are CONTROLLER-managed (not app-managed, so the
...
/** purge carries the SAME ports as offboard: the lifecycle set (registry + per-cluster resolver +
  ...
  return [
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        const { commit, unitRemoved } = await ports.registry.removeRegistration(t.stage, t.name, ctx.runId);
    ...
    },
    {
      ...
      run: async (ctx) => {
        // Controller-managed (never pruned by the pointer removal) — delete it explicitly. Idempotent:
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        try {
          const { argoNamespace } = await ports.resolver.resolve(t.clusterId);
          const stageOnly = await unitStaysRegistered(ctx, ports.registry, t, "the build-namespace grants");
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        const t = loadPurgeTarget(ctx.db, p);
        if (await unitStaysRegistered(ctx, ports.registry, t, "the build webhook")) return;
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        const t = loadPurgeTarget(ctx.db, p);
        const unitApex = unitApexFromChain(await ports.registry.readClusterValueFiles(t.domain, t.stage));
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        const t = loadPurgeTarget(ctx.db, p);
        if (await unitStaysRegistered(ctx, ports.registry, t, "the release kit")) return;
        const row = findAppRow(ctx.db, t);
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        // build-plane Vault — the one place the seed wrote — via the CONTROLLER's Vault adapter
        ...
        // recorded one.
        const t = loadPurgeTarget(ctx.db, p);
        if (!(await unitStaysRegistered(ctx, ports.registry, t, "the repo PAT"))) {
