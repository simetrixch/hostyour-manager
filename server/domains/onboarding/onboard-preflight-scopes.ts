...
export function preflightScopesStep(ports: OnboardPorts, p: OnboardParams): Step {
  return {
    ...
    run: async (ctx) => {
      ...
      // unwired client is a controller misconfiguration, never a silent skip.
      if (!ports.github) {
        throw errValidation(`onboard "${p.consumerName}" requires the GitHub client to pre-flight the consumer PAT scopes but none is wired on this controller — refusing to onboard without verifying ${REQUIRED_CONSUMER_PAT_SCOPES.join(" + ")} up front`);
      }
