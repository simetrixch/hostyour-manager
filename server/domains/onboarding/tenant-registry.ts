...
import { serializePointer, makeRegistrationGuard, trailer, type ClusterStageResolver, assertClusterStage } from "./registry.ts";
 ...
 *  controller-side render both want the current tip.
 *
...
export class TenantRegistry {
   ...
   *  registry's flat "key: <json>" serializer, which is a SUBSET of YAML, so it is read with a real
   ...
   *  consumer registry makes). Never a prune — the ServiceClaims survive, which is what keeps the
