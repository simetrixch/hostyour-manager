...
// The owner identities over HTTP (hostyour-manager#219), served by the unit plugin under /api/unit: the list, and one PUT per
...
export function registerOwnerRoutes(app: Hono<AppEnv>, deps: OwnerDeps): void {
  app.get("/owners", async (c) => c.json({ owners: await listOwnerIdentities(deps, c.req.raw.signal) } satisfies OwnersListView));

  app.put("/owners/:org/packages-reader", async (c) => {
  ...
  app.put("/owners/:org/repository-pat", async (c) => {
    const org = assertOrgLogin(c.req.param("org"));
    const parsed = CredentialBody.safeParse(await c.req.json().catch(() => ({})));
  ...
  app.delete("/owners/:org/packages-reader", async (c) => {
    await forgetOwnerCredential(deps, assertOrgLogin(c.req.param("org")), "packages-reader");
    return c.json({ ok: true });
  });

  app.delete("/owners/:org/repository-pat", async (c) => {
