...
import { composeReport, gateG16, gateG17, gateG18, gateG19, gateG23, PLATFORM_NAMESPACES } from "./compose.ts";
...
describe("gateG17 repo access (hard)", () => {
  it("passes on a successful clone and fails without leaking a credential", () => {
    expect(gateG17({ ok: true, detail: "clone succeeded" }).status).toBe("pass");
    const denied = gateG17({ ok: false, detail: "clone failed: authentication required" });
...
describe("gateG23 unit name (hard)", () => {
  ...
  it("refuses a name in the derived build-namespace space, naming the unit whose build namespace it is", () => {
    ...
    // pinned into example-auth's build namespace — where the shared registry push credential and
    // example-auth's repo PAT are materialized.
    const g = gateG23({ unitName: "example-auth-build", tenantSubdomains: [] });
    ...
    expect(gateG23({ unitName: "nobody-yet-build", tenantSubdomains: [] }).status).toBe("fail");
  });

  it("refuses Kubernetes' kube- prefix", () => {
    for (const name of ["kube-public", "kube-node-lease"]) {
      expect(gateG23({ unitName: name, tenantSubdomains: [] }).status).toBe("fail");
    }
  });
  ...
  it("refuses every shared ArgoCD project name — the per-unit AppProject is named by the unit", () => {
    for (const name of RESERVED_PROJECT_NAMES) {
      expect(gateG23({ unitName: name, tenantSubdomains: [] }).status, `"${name}" must be refused`).toBe("fail");
    }
  });
  ...
  it("refuses a name a tenant already stands on — its IdP scopes every session cookie to that exact host", () => {
    ...
    const g = gateG23({ unitName: "simetrix", tenantSubdomains: ["simetrix", "other"] });
    expect(g.status).toBe("fail");
    expect(g.found).toContain("session cookie");
    expect(gateG23({ unitName: "acme", tenantSubdomains: ["simetrix"] }).status).toBe("pass");
  });

  it("fails the whole set of gates — it is a HARD gate", () => {
    const g23 = gateG23({ unitName: "mongodb", tenantSubdomains: [] });
    expect(composeReport(runnerReport(), [gateG17({ ok: true, detail: "ok" }), g23]).verdict).toBe("fail");
  });
});

describe("gateG16 build-name uniqueness (hard)", () => {
  it("passes when no other unit claims the declared names", () => {
    const g = gateG16({
      unitName: "hostyour-manager",
      buildNames: ["controller", "gate-runner"],
  ...
  it("passes a name that neither equals its unit nor is prefixed by it — the old ownership law is gone", () => {
    ...
    // controller to hostyour-manager. Uniqueness is checked against the other units, not constructed.
    const g = gateG16({ unitName: "example-platform", buildNames: ["example-engine", "example-ui", "example-web"], foreignBuilds: [] });
    expect(g.status).toBe("pass");
  });
  ...
  it("fails the whole set of gates — it is a HARD gate", () => {
    const g16 = gateG16({ unitName: "unit-b", buildNames: ["shared-api"], foreignBuilds: [{ unit: "unit-a", build: "shared-api" }] });
    expect(composeReport(runnerReport(), [gateG17({ ok: true, detail: "ok" }), g16]).verdict).toBe("fail");
  });
});

describe("gateG18 build declaration (hard, two halves)", () => {
  it("manifest half: an empty builds[] fails, chart or no chart", () => {
    for (const c of [null, chart([], chartPins())]) {
      const g = gateG18({ declaredBuilds: [], chart: c });
      expect(g.status).toBe("fail");
      expect(g.severity).toBe("hard");
  ...
  it("chart half: passes when values-<stage>.yaml pins every declared build", () => {
    const g = gateG18({ declaredBuilds: ["acme-api"], chart: chart(["acme-api"], chartPins("acme-api")) });
    expect(g.status).toBe("pass");
    expect(g.reason).toBeNull();
  ...
  it("chart half: fails naming the declared build the chart does not pin", () => {
    const g = gateG18({ declaredBuilds: ["acme-api", "acme-worker"], chart: chart(["acme-api", "acme-worker"], chartPins("acme-api")) });
    expect(g.status).toBe("fail");
  ...
  it("a build-only unit is EMITTED, not skipped: the manifest half holds and the chart half says it has no object", () => {
    const g = gateG18({ declaredBuilds: ["controller", "gate-runner"], chart: null });
    expect(g.status).toBe("pass");
    expect(g.found).toContain("build-only — no chart to check");
  });
});

describe("gateG19 fqdn grant (hard)", () => {
  const foreign = [{ unit: "unit-a", stage: "prod" as const, fqdn: "shop.example.org" }];

  it("passes — and says so — when no fqdn is declared", () => {
    const g = gateG19({ unitName: "acme", fqdn: null, unitApex: null, clusterDomain: null, foreignFqdns: [] });
    expect(g.status).toBe("pass");
    expect(g.severity).toBe("hard");
  ...
  it("passes a declared fqdn nobody serves: outside the apex, attested by no other unit", () => {
    const g = gateG19({ unitName: "acme", fqdn: "app.acme.example.org", unitApex: "units.example.com", clusterDomain: "m1.example.com", foreignFqdns: foreign });
    expect(g.status).toBe("pass");
    expect(g.found).toContain("app.acme.example.org");
  });

  it("fails naming the unit AND the stage file that already attest the fqdn", () => {
    const g = gateG19({ unitName: "acme", fqdn: "shop.example.org", unitApex: "units.example.com", clusterDomain: null, foreignFqdns: foreign });
    expect(g.status).toBe("fail");
    expect(g.found).toContain("unit-a");
  ...
  it("fails anything under (or equal to) the cluster's unitApex — those names are the platform's own composition", () => {
    for (const fqdn of ["other.units.example.com", "units.example.com"]) {
      const g = gateG19({ unitName: "acme", fqdn, unitApex: "units.example.com", clusterDomain: null, foreignFqdns: [] });
      expect(g.status).toBe("fail");
    ...
    expect(gateG19({ unitName: "acme", fqdn: "not-units.example.com", unitApex: "units.example.com", clusterDomain: null, foreignFqdns: [] }).status).toBe("pass");
  });
  ...
  it("fails anything under (or equal to) the cluster's own FQDN — the platform's infrastructure hostnames live there and have no registration", () => {
    ...
    for (const fqdn of ["vault.m1.internal.example", "m1.internal.example"]) {
      const g = gateG19({ unitName: "acme", fqdn, unitApex: "customers.example", clusterDomain: "m1.internal.example", foreignFqdns: [] });
      expect(g.status).toBe("fail");
    ...
    expect(gateG19({ unitName: "acme", fqdn: "not-m1.internal.example", unitApex: "customers.example", clusterDomain: "m1.internal.example", foreignFqdns: [] }).status).toBe("pass");
  });
  ...
  it("fails the whole set of gates — it is a HARD gate", () => {
    const g19 = gateG19({ unitName: "acme", fqdn: "shop.example.org", unitApex: null, clusterDomain: null, foreignFqdns: foreign });
    expect(composeReport(runnerReport(), [gateG17({ ok: true, detail: "ok" }), g19]).verdict).toBe("fail");
  });
});

describe("composeReport", () => {
  it("appends controller gates and keeps the runner's reportHash untouched", () => {
    const c = composeReport(runnerReport(), [gateG17({ ok: true, detail: "clone succeeded" })]);
    expect(c.gates.map((g) => g.id)).toEqual(["G1", "G17"]);
    expect(c.reportHash).toBe("runner-hash");
    expect(c.verdict).toBe("pass");
  });

  it("flips the verdict to fail when a hard controller gate fails, even if the runner passed", () => {
    const c = composeReport(runnerReport(), [gateG17({ ok: false, detail: "clone failed" })]);
    expect(c.verdict).toBe("fail");
  });

  it("stays fail when the runner failed even if every controller gate passes", () => {
    const c = composeReport(runnerReport({ verdict: "fail" }), [gateG17({ ok: true, detail: "clone succeeded" })]);
    expect(c.verdict).toBe("fail");
  });
});
