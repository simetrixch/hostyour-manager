// gate-runner/src/gates/image-discipline.gate.test.ts
import { describe, it, expect } from "vitest";
import { imageDisciplineGate } from "./image-discipline.gate.ts";
...
describe("imageDisciplineGate image discipline", () => {
  it("passes a clean context: tagged images, a host:port tag, and a digest-only ref", () => {
    ...
    const r = imageDisciplineGate.check(ctx);
  ...
  it("fails an untagged image (implicit :latest)", () => {
    const r = imageDisciplineGate.check(makeCtx([deployment(["nginx"])]));
  ...
  it("fails an explicit :latest tag", () => {
    const r = imageDisciplineGate.check(makeCtx([deployment(["nginx:latest"])]));
    expect(r.status).toBe("fail");
    expect(r.reason).not.toBeNull();
  ...
  it("passes a :latest tag that also carries an @sha256 digest", () => {
    ...
    const r = imageDisciplineGate.check(makeCtx([deployment([ref])]));
    expect(r.status).toBe("pass");
    expect(r.reason).toBeNull();
  });
  ...
  it("fails a mutable image hiding in initContainers", () => {
    ...
    const r = imageDisciplineGate.check(makeCtx([raw]));
    expect(r.status).toBe("fail");
  ...
  it("fails a :latest ref whose only digest is a bogus @0:0", () => {
    const r = imageDisciplineGate.check(makeCtx([deployment(["app:latest@0:0"])]));
    expect(r.status).toBe("fail");
    expect(r.reason).not.toBeNull();
  ...
  it("fails a :latest ref whose @sha256 digest is too short to be real", () => {
    const r = imageDisciplineGate.check(makeCtx([deployment(["app:latest@sha256:deadbeef"])]));
    expect(r.status).toBe("fail");
    expect(r.reason).not.toBeNull();
