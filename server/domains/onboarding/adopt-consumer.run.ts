...
// never "controller" (which states a gate-validated onboard ran); (2) the run PROBES THE LIVE CLUSTER
...
/** adopt-consumer drives the SAME narrow port set as suspend/resume (LifecyclePorts): the registry
  ...
  return {
    ...
    plan: async (params, { db }) => {
      ...
      return {
        ...
        // The registration read rides the BOOKS worktree (registry.fetchResetBranch hard-resets it),
        ...
        locks: [{ resource: "git-branch", key: ports.registry.branch }, { resource: "git-branch", key: t.domain }],
        warnings: [],
        requiredSecrets: [],
      };
    },
