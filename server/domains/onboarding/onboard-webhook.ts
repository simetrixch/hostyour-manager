...
// stood on another cluster answers to no address this controller composes today — and the removal is
...
// SECRET) is read from the controller config — the seeder is write-only, so the controller cannot read
...
export function setupWebhookStep(ports: OnboardPorts, p: OnboardParams): Step {
  return {
    ...
    run: async (ctx) => {
      ...
      // build trigger), so an unwired GitHub client is a controller misconfiguration, never a silent skip.
      if (!ports.github) {
        throw errValidation(`onboard "${p.consumerName}" requires the GitHub consumer client but none is wired on this controller — refusing to onboard a consumer that can never build (no hook → no build)`);
      }
      ...
      if (!secret) {
        throw errValidation(`onboard "${p.consumerName}" cannot set up the build webhook — GITHUB_WEBHOOK_SECRET is not configured on this controller (the EventListener validates X-Hub-Signature-256 against it; a hook without the matching secret never triggers a build)`);
      }
...
export function removeWebhookCleanup(ports: OnboardPorts, p: OnboardParams): Cleanup {
  return {
    ...
    run: async (ctx) => {
      ...
      if (await unitStaysRegistered(ctx, ports.registry, unit, "the build webhook")) return;
 ...
 *  another cluster would never equal the address this controller composes today, and nothing else would
...
export async function removeConsumerWebhook(
  ...
  if (!opts.github) {
    ctx.log("meta", `webhook removal skipped for ${opts.consumerName} — no GitHub consumer client is wired on this controller`);
    return;
  }
