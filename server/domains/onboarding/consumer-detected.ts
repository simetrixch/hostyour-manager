...
// perfectly while the Controller cannot see it: the Consumers list is a projection of the apps rows,
...
// registration beside it is leftover state — or a re-deploy the Controller never learned of — not an
// accounted-for consumer.
//
// Boundary: domain layer — inventory reads, the consumer Registry's registration scan, and (for the
...
export async function scanDetectedConsumers(deps: { db: Db; registry: Registry }): Promise<Pick<DetectedScan, "detected" | "skipped">> {
  ...
  for (const cluster of active) {
    ...
    const scan = await registry.listConsumerRegistrations(cluster.domain, cluster.stage);
 ...
 *  diff uses, so a consumer the Controller already lists is not reported twice. The registration side
...
export async function scanClusterOrphanConsumers(deps: {
  ...
  const { db, registry, resolver } = deps;
  ...
  for (const cluster of active) {
    ...
    try {
      ...
      const registered = await registry.listConsumerRegistrations(cluster.domain, stage);
