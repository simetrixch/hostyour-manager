...
import { renderSmtpOpsGrant } from "#unit/server/build-rbac.ts";
...
describe("purge run definition", () => {
  ...
  it("purging ONE stage of a two-stage unit keeps everything the other stage needs", async () => {
    ...
    for (const deploy of [{ stage: "prod" as const, cluster: "s1" }, { stage: "dev" as const, cluster: "s2" }]) {
      await reg.commitRegistration({ unit, builds: ["acme"], deploy: { ...deploy, host: "acme", chartPath: "deploy/chart", databases: [], keyPatterns: [], channelPatterns: [], services: [], size: "small", mongodb: "shared", quota: seedQuota("small") }, runId: `run_onb_${deploy.stage}` });
    }
