...
// name. It mirrors tenant-offboard.run.ts's offboardSteps shape — its own steps are named remove ->
  ...
  return [
    {
      ...
      run: async (ctx) => {
        ...
        const { commit } = await ports.registrations.removeTenant(t.stage, t.guid, ctx.runId);
