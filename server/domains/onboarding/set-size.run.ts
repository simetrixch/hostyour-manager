...
export async function consumerComposition(
  registry: Pick<LifecyclePorts["registry"], "readRegistration">,
  ac: { stage: Stage; name: string },
): Promise<UnitComposition> {
  const reg = await registry.readRegistration(ac.stage, ac.name);
  ...
  return [
    attestTargetStep(ports, p.appId),
    {
      ...
      run: async (ctx) => {
        ...
        const quota = resolveUnitQuota(ctx.db, p.size, await consumerComposition(ports.registry, ac));
        const { commit } = await ports.registry.setQuota(ac.stage, ac.name, quota, ctx.runId);
...
export function makeSetSizeDef(ports: LifecyclePorts): RunDefinition<SetSizeParams> {
  return {
    ...
    plan: async (params, { db }) => {
      ...
      const quota = resolveUnitQuota(db, params.size, await consumerComposition(ports.registry, ac));
  ...
  return [
    attestTenantTargetStep(ports, p.tenantId),
    {
      ...
      run: async (ctx) => {
        ...
        const { commit } = await ports.registry.setQuota(tc.stage, tc.guid, quota, ctx.runId);
        ctx.checkpoint({ commit, size: p.size, quota });
