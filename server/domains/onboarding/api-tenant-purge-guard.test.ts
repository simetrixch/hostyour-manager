 ...
 *  s1/prod, so a single-stage stand-in is all TenantRegistry needs to satisfy commitTenant's stage
 *  boundary check. */
const CLUSTER_STAGE: ClusterStageResolver = async (cluster) => ({ name: cluster, stage: "prod" });

/** The tenant as it stands in GitOps — committed through the real registry, so the guard reads exactly
...
function lifecyclePorts(registry: TenantRegistry, clusterReader: FakeClusterReader): TenantLifecyclePorts {
  return {
    registry,
...
/** The tenant routes with the purge family wired — the registry rides with it exactly as it does in
 *  production (buildTenantOnboarding constructs both or neither), because the guard reads the pointer
 *  through the very registry the runs commit through. The cluster reader rides out too: the two deletes
 *  that make a purge destructive (Tenant CR + namespace) record on it, so a test can prove a refused
 *  purge issued neither. */
async function makeTenant(): Promise<{ app: Hono<AppEnv>; executor: Executor; cookie: string; registry: TenantRegistry; cluster: FakeClusterReader }> {
  const store = new CredentialStore({ db: db.db, logger });
  const reg = new TenantRegistry(new FakePlatformRepo(), CLUSTER_STAGE);
  const cluster = new FakeClusterReader({ deployState: { domain: "s1.example", stage: "prod", writtenAt: "x", generation: 1 } });
  const executor = new Executor({ db: db.db, creds: store, bus: new RunEventBus(), logger, registry: buildRegistry({ db: db.db }, [makeTenantPurgeDef(lifecyclePorts(reg, cluster))]), sshFactory: noSsh, actor: () => "op_system" });
  const session = new SessionCodec(db.db, config);
  const app = createApp({
    config, logger, getReadiness: () => ({ ok: true, checks: [] }), session,
    registerAuth: () => undefined,
    registerProtected: (a) => registerTenantRoutes(a, { executor, db: db.db, onboardingEnabled: true, registry: reg }),
  });
  const cookie = await session.mint({ sub: "op_test", groups: ["admins"], via: "oidc" });
  return { app, executor, cookie, registry: reg, cluster };
}
...
describe("POST /api/tenants/purge refuses a tenant that is still LIVE", () => {
  it("an ACTIVE row whose pointer still stands is refused, and NO run is planned", async () => {
    ...
    const { app, cookie, registry } = await makeTenant();
    await registry.commitTenant({ stage: "prod", guid: GUID, registration: registration(), runId: "run_onb" });
  ...
  it("a SUSPENDED row is refused too — suspend keeps the Tenant CR, the databases and the Vault path", async () => {
    ...
    const { app, cookie, registry } = await makeTenant();
    await registry.commitTenant({ stage: "prod", guid: GUID, registration: registration({ suspended: true }), runId: "run_onb" });
    await expectLiveRefusal(await purge(app, cookie));
  });
  ...
  it("PROVISIONING is allowed — the unfinished create-tenant is exactly what a purge is the remedy for", async () => {
    ...
    await registry.commitTenant({ stage: "prod", guid: GUID, registration: registration(), runId: "run_onb" }); // its pointer landed before the run died
  ...
  it("an UNREADABLE pointer beside a live row is refused — a body nobody can parse is no proof of removal", async () => {
    ...
    seedTenantRow("active");
    const { app, cookie, registry } = await makeTenant();
    registry.scanTenant = () => Promise.resolve({ status: "unreadable", reason: `tenants/prod/${GUID}/tenant.yaml failed its schema: cluster Invalid input` });
    await expectLiveRefusal(await purge(app, cookie));
  });
  ...
  it("the guard reads no pointer at all when there is no live row — the orphan path stays a pure DB read", async () => {
    ...
    const { app, executor, cookie, registry } = await makeTenant();
    let scans = 0;
    const scan = registry.scanTenant.bind(registry);
    registry.scanTenant = (stage, guid) => { scans += 1; return scan(stage, guid); };
    const { runId } = (await (await purge(app, cookie)).json()) as { runId: string };
    await executor.settle(runId);
    expect(getRun(db.db, runId)?.status).toBe("planned");
...
describe("the same refusal is BELTED in attest-target, so approving a stale plan cannot deprovision", () => {
  it("a purge planned against a PROVISIONING tenant is refused when that tenant has meanwhile gone live", async () => {
    ...
    seedCluster();
    seedTenantRow("provisioning");
    const { app, executor, cookie, registry, cluster } = await makeTenant();
    await registry.commitTenant({ stage: "prod", guid: GUID, registration: registration(), runId: "run_onb" });
    const { runId } = (await (await purge(app, cookie)).json()) as { runId: string };
    await executor.settle(runId);
    expect(getRun(db.db, runId)?.status).toBe("planned");
  ...
  it("lets the purge it was planned for through — the belt admits it and every reapable artifact goes", async () => {
    ...
    await registry.commitTenant({ stage: "prod", guid: GUID, registration: reg, runId: "run_onb" });
    const { runId } = (await (await purge(app, cookie)).json()) as { runId: string };
    await executor.settle(runId);
    await executor.approve(runId);
    await executor.settle(runId);
    ...
    expect(await registry.readTenant("prod", GUID)).toBeNull();
