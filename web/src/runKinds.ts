 ...
 *
 *  Filter by KIND, never by targetId: a `consumer-onboard` run freezes targetKind "cluster" + targetId a
 *  clusterId (the app row does not exist yet at plan time), while offboard/suspend/resume target the
 *  app — so a `targetId === consumerId` filter would drop every consumer-onboard run AND mismatch.
 *  `consumer-adopt` too: it targets the CLUSTER (a detected consumer has no app row to target). */
 ...
 *  read from the same RUN_FAMILY declaration. Same KIND-not-targetId rule: a `tenant-create` run freezes
 *  targetKind "cluster" + targetId a clusterId (the tenant row does not exist yet at plan time), while
 *  tenant-add-app/tenant-remove-app + the tenant lifecycle target the tenant — so a `targetId === tenantId` filter
 *  would drop every tenant-create run. tenant-purge belongs here for the SAME reason `consumer-purge` belongs to
 ...
 *  whole families and this one never can be, since cluster-deploy-slave and cluster-redeploy belong to the same family
