...
async function readLoginServer(ports: TailnetPorts, domain: string): Promise<string> {
  if (!ports.platformRepo) {
    throw errNotConfigured(
      ...
      `${clusterMapPath(domain)}) off this installation's books branch, and reaching it needs GITHUB_REPO + ` +
  ...
  // THE BOOKS, WHICH IS WHERE EVERY MAP OF AN INSTALLATION STANDS. A cluster carrying only the slave
  // part has no branch of its own, so a read off a branch named after the cluster would find nothing
  // for exactly the machines a rejoin is run against.
  const books = ports.platformRepo.booksBranch;
  const raw = await ports.platformRepo.withBranch(books, (branch) => branch.readFile(mapPath));
  if (raw === null) {
    throw errValidation(
      `no ${mapPath} on ${books} — an installation keeps every cluster map on that one branch, so a ` +
      "map missing there is a cluster nothing has marked",
    );
  }
  ...
  if (url === undefined) {
    throw errValidation(
      `${mapPath} on ${books} carries no readable global.endpoints.tailnet.url — the coordinator's ` +
      "address is rendered into every cluster map (https://tale.<master>), and a rejoin refuses to guess it",
    );
  }
  return url;
}
