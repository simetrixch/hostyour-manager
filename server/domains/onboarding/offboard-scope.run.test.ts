...
async function seedTwoStages(reg: Registry): Promise<void> {
...
describe("offboard scope — one stage of a two-stage unit", () => {
  it("leaves the other stage everything it needs to build, release and deploy", async () => {
    ...
    const reg = new Registry(platform, twoStageClusterStage);
    ...
    const prt: OffboardPorts = {
      registry: reg, resolver: twoClusterResolver(projects), argoWatchTimeoutMs: 1000,
  ...
  it("removes what the stages shared once the LAST stage is offboarded", async () => {
    ...
    const prt: OffboardPorts = {
      registry: reg, resolver: twoClusterResolver(new FakeMasterProjectWriter()), argoWatchTimeoutMs: 1000,
