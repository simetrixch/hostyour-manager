...
function deploy(over: Partial<{ stage: Stage; chartPath: string; cluster: string; host: string; databases: string[]; keyPatterns: string[]; channelPatterns: string[]; services: ConsumerRegistration["services"]; size: "small" | "medium" | "large"; mongodb: "shared" | "standalone" | "replicaset"; quota: UnitQuota; fqdn: string }> = {}) {
  return { stage: "prod" as Stage, chartPath: "deploy/chart", cluster: "s1", host: "acme", databases: [], keyPatterns: [], channelPatterns: [], services: [], size: "small" as const, mongodb: "shared" as const, quota: seedQuota("small"), ...over };
}
...
describe("ConsumerRegistrationSchema", () => {
  ...
  it("accepts the stage form and requires the WHOLE deploy group, services and size included", () => {
    expect(ConsumerRegistrationSchema.safeParse({ ...unit(), chartPath: "deploy/chart", cluster: "s1", host: "acme", databases: [], keyPatterns: [], channelPatterns: [], services: [], size: "small", mongodb: "shared", quota: seedQuota("small") }).success).toBe(true);
  ...
  it("refuses builds[] in a stage registration", () => {
    const r = ConsumerRegistrationSchema.safeParse({ ...unit(), chartPath: "deploy/chart", cluster: "s1", host: "acme", databases: [], keyPatterns: [], channelPatterns: [], services: [], size: "small", mongodb: "shared", quota: seedQuota("small"), builds: ["acme-backend"] });
    expect(r.success).toBe(false);
...
describe("serializePointer (generic over the registration schemas)", () => {
  ...
  it("carries the literal databases[] and the claimed services[] verbatim", () => {
    const { stage: _stage, ...group } = deploy({ databases: ["example_auth"], keyPatterns: [], channelPatterns: [], services: ["postgresql"] });
...
const bootstrapBuildYaml = (suspended: string): string => `# The platform's own Manager, registered as a unit so it is built the way everything else is.
#
...
describe("Registrations.commitRegistration", () => {
  ...
  it("NEVER puts a deploy-group field in build.yaml", async () => {
    ...
    await new Registrations(repo).commitRegistration({
      unit: unit(),
      builds: ["acme-backend"],
      deploy: deploy({ databases: ["example_auth"], keyPatterns: [], channelPatterns: [], services: ["postgresql"] }),
      runId: "run_1",
    });
...
describe("Registrations.listAttestedBuildNames", () => {
  ...
  // wire-units.ts:615's exceptUnit-less call, the first tenant too. It stops the day that
