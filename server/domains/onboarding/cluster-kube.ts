// The per-cluster kube resolver. The controller pod runs ON the master with only
...
//    controller's own host): return the master-local trio (built from the master kube input —
...
// credential opener ride in as injected deps (wire-onboarding.ts supplies the real ones), keeping
 ...
 *  and the credential store. wire-onboarding.ts supplies: `db` (the controller DB), the pre-built
...
function readSlavePlane(planeJson: unknown, name: string): SlavePlane {
  ...
  if (read.kind === "unsupported") {
    throw errValidation(`cluster "${name}" carries a management plane of version ${read.v}, which this controller cannot read — deploy it with a controller that knows that version`);
  }
...
export function makeClusterKubeResolver(deps: ClusterKubeDeps): ClusterKubeResolver {
  return {
    async resolve(clusterId: string): Promise<ResolvedClusterKube> {
      ...
      // cluster, and a harvested cluster-admin bearer for the host the controller runs on would be a
