// gate-runner/src/gates/database-sync-ordering.gate.test.ts
...
// consumer (no such waves, no such hooks). Each context is hand-built and minimal (the data-protection gate's idiom).
import { describe, expect, it } from "vitest";
import { databaseSyncOrderingGate } from "./database-sync-ordering.gate.ts";
...
describe("databaseSyncOrderingGate database sync ordering", () => {
  it("PASSES swissbookai's compliant chart: lowest wave -3, migrate Job a plain wave -1 resource (not a PreSync hook)", () => {
    ...
    const r = databaseSyncOrderingGate.check(makeCtx(rendered));
  ...
  it("PASSES wave -10 exactly (the floor is inclusive)", () => {
    const r = databaseSyncOrderingGate.check(makeCtx([doc("Job", "acme-setup", { [WAVE]: "-10" })]));
    expect(r.status).toBe("pass");
    expect(r.reason).toBeNull();
  });
  ...
  it("REJECTS a consumer resource at wave -21 (claiming the platform's own database wave)", () => {
    const r = databaseSyncOrderingGate.check(makeCtx([doc("Deployment", "acme-db-shadow", { [WAVE]: "-21" })]));
  ...
  it("REJECTS the -11 boundary (one below the inclusive floor)", () => {
    const r = databaseSyncOrderingGate.check(makeCtx([doc("Job", "acme-early", { [WAVE]: "-11" })]));
    expect(r.status).toBe("fail");
    expect(r.reason).toContain("-11");
  });

  it("REJECTS an UNQUOTED numeric wave below the floor (YAML parsed it as a number, not a string)", () => {
    const r = databaseSyncOrderingGate.check(makeCtx([doc("Deployment", "acme-x", { [WAVE]: -21 })]));
    expect(r.status).toBe("fail");
    expect(r.reason).toContain("-21");
  });
  ...
  it("REJECTS a PreSync hook that references the database Secret postgresql-credentials", () => {
    const r = databaseSyncOrderingGate.check(makeCtx([doc("Job", "acme-db-init", { [HOOK]: "PreSync" }, readsDbSecret())]));
    expect(r.status).toBe("fail");
    expect(r.reason).not.toBeNull();
  ...
  it("REJECTS a PreSync hook declared with a multi-phase annotation (PreSync,PostSync) that depends on the database", () => {
    const r = databaseSyncOrderingGate.check(makeCtx([doc("Job", "acme-db-init", { [HOOK]: "PreSync,PostSync" }, readsDbSecret())]));
    expect(r.status).toBe("fail");
    expect(r.reason).toContain("PreSync");
  });
  ...
  it("PASSES a PreSync hook that does NOT touch the database (the image-guard) — the no-op for existing PreSync hooks", () => {
    ...
    const r = databaseSyncOrderingGate.check(makeCtx([guard]));
    expect(r.status).toBe("pass");
    expect(r.reason).toBeNull();
  });
  ...
  it("PASSES a plain (non-hook) resource that references the database at a valid wave — only PreSync hooks are policed for DB deps", () => {
    const r = databaseSyncOrderingGate.check(makeCtx([doc("Job", "acme-migrate", { [WAVE]: "-1" }, readsDbSecret())]));
    expect(r.status).toBe("pass");
    expect(r.reason).toBeNull();
  });
  ...
  it("does not crash on a hostile doc whose metadata is a non-object", () => {
    ...
    const r = databaseSyncOrderingGate.check(makeCtx([hostile]));
  ...
  it("ignores a non-integer sync-wave value (not a floor violation)", () => {
    const r = databaseSyncOrderingGate.check(makeCtx([doc("Deployment", "acme-x", { [WAVE]: "not-a-number" })]));
    expect(r.status).toBe("pass");
    expect(r.reason).toBeNull();
  });
});
