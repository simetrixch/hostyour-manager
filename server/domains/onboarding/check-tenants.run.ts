...
//
// THE TOKEN COMES OFF THE CLUSTER, NOT OUT OF VAULT. The controller writes a tenant's crypto entry
...
import { BOOTSTRAP_TOKEN_KEY, TENANT_APP_SECRET } from "./tenant-admin-invite.ts";
...
/** No parameters: the check is over every tenant this controller knows, and a check that could be
    ...
    run: async (ctx: StepCtx) => {
      ...
      for (const t of ask) {
        ...
        try {
          ...
          // it — and the read uses the per-cluster credential this controller already holds.
          ...
          if (!token) {
            state = "unreachable";
            because = `the bootstrap token (Secret ${TENANT_APP_SECRET} key ${BOOTSTRAP_TOKEN_KEY}) is absent in ${ns}`;
          } else {
