...
const DBTOOLS_IMAGE = "registry.example/dbtools:1.0.0";
...
export function consumerPorts(f: RelocationFakes): ConsumerRelocationPorts & { registry: Registry } {
  const registry = new Registry(f.platformRepo, marked);
  return {
    registry,
    resolver: f.resolver,
...
export function tenantPorts(f: RelocationFakes): TenantRelocationPorts & { registry: TenantRegistry } {
  const registry = new TenantRegistry(f.platformRepo, marked);
  return {
    registry,
    resolver: f.resolver,
    argoWatchTimeoutMs: 1000,
...
export async function seedTenantWorld(registry: TenantRegistry): Promise<void> {
  await registry.commitTenant({ stage: "prod", guid: GUID, registration: tenantEntry(), runId: "run_onb" });
}
