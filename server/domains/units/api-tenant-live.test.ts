...
function seedTenant(over: { status?: TenantStatus; suspended?: boolean } = {}): void {
  seedCluster();
  db.db.insert(tenants).values({ id: "tnt_1", clusterId: "cls_1", guid: TGUID, subdomain: "acme", stage: "prod", members: ["auth", "jobs", "report"], identityProvider: "auth", provenance: "manager", status: "active", ...over }).run();
...
describe("tenant live reconciliation (GET /api/tenants/:id/live)", () => {
  ...
  it("a PROVISIONING tenant's app rows still count toward the expected set — a half-created tenant reads Missing, not Healthy", async () => {
    ...
    seedCluster();
    db.db.insert(tenants).values({ id: "tnt_1", clusterId: "cls_1", guid: TGUID, subdomain: "acme", stage: "prod", members: ["auth", "jobs", "report"], identityProvider: "auth", provenance: "manager", status: "provisioning" }).run();
