...
// it, so stating it once is what stops one screen from dropping a tenant the other still has a verb for.
//
...
//   offering the purge it had just completed — the most destructive verb in the product appearing to do
//   nothing at all.
//
...
//     — so purge is the ONLY verb that reaps them, and the purge route accepts exactly this row state.
...
//     state both removal verbs are the remedy for.
...
//     Offboard is the verb offered there, and a purge may follow it: offboard first, then purge whatever
...
//     product's most destructive verb on a tenant that has already been deprovisioned is noise that can
...
export interface TenantRowOffer {
   ...
   *  every verb that edits a live tenant (add-app, remove-app, suspend, resume, offboard) is gone from it.
 ...
 *  list so they stay reachable for the one verb they still have without crowding the tenants that run.
