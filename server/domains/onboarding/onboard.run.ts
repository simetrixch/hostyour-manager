...
//    seven platform units go through this form, which is what makes the controller the writer of
...
// comes OUT of that external cycle; the controller reads its results (the minted tag, the bump
...
export const DeployableOnboardParams = OnboardParamsBase.extend({
  ...
  // per key whether the Controller MINTS it (generate) or the operator supplied it (required).
...
/** The Controller-side clients the steps drive (master-local; no SSH). The kube clients are no
...
export interface OnboardPorts {
  ...
  runner: GateRunner;
  registry: Registry;
  /** Every subdomain a TENANT stands at (TenantRegistry over catalog — a second repo, hence a
   ...
   *  controller cannot watch the release run), never a silent skip (setup-webhook precedent). */
...
function deployableSteps(ports: OnboardPorts, p: DeployableOnboardParams): Step[] {
  ...
  const steps: Step[] = [
    ...
    // The provisioning block: the four per-unit objects the Controller writes outside
...
function buildOnlySteps(ports: OnboardPorts, p: BuildOnlyOnboardParams): Step[] {
  ...
  return [
    ...
    // them the EventListener cannot create the release PipelineRun and the controller cannot watch
...
export interface ResolvedTarget {
   ...
   *  (Registry.readClusterValueFiles) and completes the OnboardTarget. */
    ...
    planStream: async (rawParams, ctx) => {
      ...
      if (req.clusterId === undefined) {
        ...
        const outcome = await validateOnboard(
          ...
          { repo: ports.repo, runner: ports.runner, registry: ports.registry, tenantSubdomains: ports.tenantSubdomains, log: ctx.log, signal: ctx.signal, attestListening: ports.attestListening, ...(ports.validationBudgetMs !== undefined ? { pollBudgetMs: ports.validationBudgetMs } : {}) },
        );
        ...
        const plan: Plan = {
          ...
          locks: [
            { resource: "git-branch", key: ports.registry.branch }, // the registration commit's branch
      ...
      const clusterValueFiles = await ports.registry.readClusterValueFiles(r.target.domain, r.target.stage);
