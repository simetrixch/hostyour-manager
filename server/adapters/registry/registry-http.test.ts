...
// rides a real temp file, exactly the mounted manager-registry-pull dockerconfigjson shape.
...
describe("HttpRegistryProbe", () => {
  ...
  it("fails CLOSED when the pull credential file is absent (fresh install before ESO synced) — fetch never fires", async () => {
    ...
    await expect(probe.imageExists(ref)).rejects.toThrow(/manager-registry-pull/);
    expect(fetchSpy).not.toHaveBeenCalled();
  });
...
describe("HttpRegistryMaintenance", () => {
  ...
  it("listRepos reads /v2/_catalog with the push basic auth and returns repositories[]", async () => {
    ...
    expect(repos).toEqual(["manager", "library/redis"]);
  ...
  it("listRepos follows the RFC-5988 Link rel=next pagination to completion", async () => {
    const file = writeDockerConfig({ [HOST]: { auth: AUTH } });
    stubRouted({
      "GET /v2/_catalog": { status: 200, json: { repositories: ["manager"] }, headers: { link: '</v2/_catalog?n=1000&last=manager>; rel="next"' } },
      "GET /v2/_catalog?n=1000&last=manager": { status: 200, json: { repositories: ["example-engine"] } },
    });
    expect(await new HttpRegistryMaintenance(cfg(file)).listRepos()).toEqual(["manager", "example-engine"]);
  });

  it("listTags returns the tag list; a 404 (absent repo) yields [] not a throw", async () => {
    const file = writeDockerConfig({ [HOST]: { auth: AUTH } });
    stubRouted({
      "GET /v2/manager/tags/list": { status: 200, json: { name: "manager", tags: ["1.0", "2.0"] } },
      "GET /v2/gone/tags/list": { status: 404, json: { errors: [] } },
    });
    const m = new HttpRegistryMaintenance(cfg(file));
    expect(await m.listTags("manager")).toEqual(["1.0", "2.0"]);
  ...
  it("listTags fails CLOSED on an undecidable status (500) — a partial list must never be trusted", async () => {
    const file = writeDockerConfig({ [HOST]: { auth: AUTH } });
    stubRouted({ "GET /v2/manager/tags/list": { status: 500 } });
    await expect(new HttpRegistryMaintenance(cfg(file)).listTags("manager")).rejects.toMatchObject({ code: "UPSTREAM" });
  });

  it("resolveDigest returns the Docker-Content-Digest header; a missing header fails closed", async () => {
    const file = writeDockerConfig({ [HOST]: { auth: AUTH } });
    stubRouted({ "HEAD /v2/manager/manifests/1.0": { status: 200, headers: { "docker-content-digest": "sha256:beef" } } });
    expect(await new HttpRegistryMaintenance(cfg(file)).resolveDigest("manager", "1.0")).toBe("sha256:beef");

    vi.unstubAllGlobals();
    const file2 = writeDockerConfig({ [HOST]: { auth: AUTH } });
    stubRouted({ "HEAD /v2/manager/manifests/1.0": { status: 200 } }); // no digest header
    await expect(new HttpRegistryMaintenance(cfg(file2)).resolveDigest("manager", "1.0")).rejects.toMatchObject({ code: "UPSTREAM" });
  });

  it("resolveDigest fails CLOSED on a 404 (a tag we just listed vanished — a race, not an answer)", async () => {
    const file = writeDockerConfig({ [HOST]: { auth: AUTH } });
    stubRouted({ "HEAD /v2/manager/manifests/1.0": { status: 404 } });
    await expect(new HttpRegistryMaintenance(cfg(file)).resolveDigest("manager", "1.0")).rejects.toMatchObject({ code: "UPSTREAM" });
  });

  it("deleteManifest treats 202 as done and 404 as already-gone; any other status fails closed", async () => {
    const file = writeDockerConfig({ [HOST]: { auth: AUTH } });
    stubRouted({
      "DELETE /v2/manager/manifests/sha256:beef": { status: 202 },
      "DELETE /v2/manager/manifests/sha256:gone": { status: 404 },
      "DELETE /v2/manager/manifests/sha256:bad": { status: 500 },
    });
    const m = new HttpRegistryMaintenance(cfg(file));
    await expect(m.deleteManifest("manager", "sha256:beef")).resolves.toBeUndefined();
    await expect(m.deleteManifest("manager", "sha256:gone")).resolves.toBeUndefined();
    await expect(m.deleteManifest("manager", "sha256:bad")).rejects.toMatchObject({ code: "UPSTREAM" });
  });
});
