...
function consumerPorts(reg: Registry): LifecyclePorts {
  return {
    registry: reg,
...
function tenantPorts(reg: TenantRegistry): TenantLifecyclePorts {
  return {
    registry: reg,
    resolver: new FakeClusterKubeResolver({
      clusterReader: new FakeClusterReader(ATTESTING),
...
async function seedConsumer(reg: Registry): Promise<void> {
  seedCluster();
...
async function seedTenant(reg: TenantRegistry): Promise<void> {
  seedCluster();
...
describe("tenant-set-size run", () => {
  it("writes the figures once, and the summary says they bound EACH member namespace", async () => {
    const reg = new TenantRegistry(new FakePlatformRepo(), async (c) => ({ name: c, stage: "prod" }));
