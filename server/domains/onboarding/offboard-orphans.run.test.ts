...
// assert-no-orphans, the offboard's one measuring step. It reads back the objects the CONTROLLER wrote
...
function seedApp(): void {
  ...
  db.db.insert(apps).values({ id: "app_1", clusterId: "cls_1", name: "acme", stage: "prod", repoUrl: REPO, chartPath: "deploy/chart", provenance: "controller", status: "active" }).run();
}
...
async function seedRegistrationsAndRemoveProd(reg: Registry, alsoDev: boolean): Promise<void> {
 ...
 *  and the registry the tree was seeded on. */
function ports(reg: Registry, world: { cluster: FakeClusterReader; projects: FakeMasterProjectWriter; buildRbac: FakeBuildRbacWriter; repoCredential: FakeRepoCredentialWriter; dns: FakeDnsProvider }): OffboardPorts {
  return {
    registry: reg,
...
describe("offboard assert-no-orphans", () => {
  it("fails the run and names every object a fail-soft delete left behind", async () => {
    seedApp();
    const reg = new Registry(new FakePlatformRepo(), twoStageClusterStage);
    ...
    // unit's address. Each of them was WRITTEN by this controller outside any chart, so nothing else
