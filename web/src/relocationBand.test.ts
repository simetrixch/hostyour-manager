...
const run = (over: Partial<RunView>): RunView =>
  ({ id: "run_1", kind: "migrate", targetKind: "app", targetId: "app_1", status: "planned", summary: "", steps: [], requiredSecrets: [], requiredInputs: [], createdAt: 0, startedAt: null, endedAt: null, deletedAt: null, ...over }) as RunView;
...
describe("relocationRun", () => {
  it("surfaces an OPEN relocation run of THIS unit, and ignores other units and other kinds", () => {
    const runs = [
      ...
      run({ id: "r_other_kind", kind: "offboard", status: "running" }),
      run({ id: "r_mine", kind: "backup", status: "planned" }),
    ];
