 ...
 *  never move a tenant across stages, so a single-stage stand-in is all TenantRegistry needs. */
const CLUSTER_STAGE: ClusterStageResolver = async (cluster) => ({ name: cluster, stage: "prod" });

function lifecyclePorts(registry: TenantRegistry, resolver: FakeClusterKubeResolver): TenantLifecyclePorts {
  return { registry, resolver, catalogRepoUrl: DEPLOY_URL, argoWatchTimeoutMs: 1000, resolveUnitApex: async () => "example.com" };
}
...
async function makeTenant(): Promise<{ app: Hono<AppEnv>; cookie: string; activator: FakeActivator }> {
  ...
  const resolver = new FakeClusterKubeResolver({
    clusterReader: new FakeClusterReader({
      ...
      secretValues: { [`${memberNamespace(GUID, "auth")}/${TENANT_APP_SECRET}/${BOOTSTRAP_TOKEN_KEY}`]: "boot_tok_abc" },
    }),
  ...
  const app = createApp({
    ...
    registerProtected: (a) => registerTenantRoutes(a, { executor, db: db.db, onboardingEnabled: true, registry: reg, resolver, activator, resolveUnitApex: async () => "example.com" }),
  });
