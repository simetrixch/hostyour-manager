...
describe("create-tenant plans the zone before the first write", () => {
  ...
  it("a replace whose old tenant serves the wildcard from another cluster is refused by G27, its registration untouched", async () => {
    ...
    const prt = ports(dns);
    await prt.registrations.commitTenant({ stage: "prod", guid: "e2e8ymj86dk8", runId: "run_old", registration: { cluster: "s2", subdomain: SUB, members: testMembers([]), identityProvider: "auth", routing: "host", ownDomain: "", apps: [], seedUsers: false, quota: seedQuota("small"), resetNonce: "1", suspended: false, quiesced: false, appsImage: "", appsImageTag: "" } });
  ...
  it("provision-dns writes the ONE wildcard as a CNAME onto the cluster, replacing a leftover and saying what stood there", async () => {
    ...
    const p = CreateTenantParams.parse({
      ...
      members: testMembers([]), identityProvider: "auth", routing: "host", ownDomain: "", owner: "team-acme", expectedApps: [], catalogRepoUrl: prt.catalogRepoUrl,
