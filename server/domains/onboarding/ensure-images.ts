// The tenant ensure-images gate. The tenant fan-out deploys ONLY pre-built images from the registry
...
/** One image the pinned fan-out requires from the cluster's registry. Frozen into the run params at
...
export function ensureImagesStep(ports: EnsureImagesPorts, p: EnsureImagesParams): Step {
  return {
    name: "ensure-images",
    title: "Check the fan-out's registry images exist",
    run: async (ctx) => {
