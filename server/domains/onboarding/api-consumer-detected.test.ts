...
/** A Registry whose registration reads REJECT — the unreadable install branch. */
const brokenRegistry = (): Registry => ({ listConsumerRegistrations: () => Promise.reject(new Error("install branch unreachable")) }) as unknown as Registry;

async function makeApp(opts: { resolver?: FakeClusterKubeResolver; registry?: Registry }): Promise<{ app: Hono<AppEnv>; cookie: string }> {
  const store = new CredentialStore({ db: db.db, logger });
  const executor = new Executor({ db: db.db, creds: store, bus: new RunEventBus(), logger, registry: buildRegistry({ db: db.db }), sshFactory: noSsh, actor: () => "op_system" });
...
describe("GET /api/consumers/detected — the two halves composed", () => {
  it("answers both halves in one response", async () => {
    const registry = new Registry(new FakePlatformRepo(), async () => ({ name: "s1", stage: "prod" }));
    const { app, cookie } = await makeApp({ resolver: resolverHolding(["ghost"]), registry });
  ...
  it("keeps the cluster orphans found on a HEALTHY cluster when another cluster's read fails", async () => {
    ...
    const { app, cookie } = await makeApp({ resolver, registry });
    const body = await scan(app, cookie);
    expect(body.error).toBeUndefined();
    expect(body.clusterOrphans.map((o) => o.name)).toEqual(["ghost"]);
  ...
  it("an unreadable install branch fails BOTH halves, and each says so in its own words", async () => {
    ...
    const { app, cookie } = await makeApp({ resolver: resolverHolding(["ghost"]), registry: brokenRegistry() });
    const body = await scan(app, cookie);
  ...
  it("degrades the CLUSTER half alone when no resolver is wired, and still answers the registration half", async () => {
    const registry = new Registry(new FakePlatformRepo(), async () => ({ name: "s1", stage: "prod" }));
    const { app, cookie } = await makeApp({ registry });
    const body = await scan(app, cookie);
    expect(body.error).toBeUndefined();
    expect(body.clusterOrphans).toEqual([]);
    expect(body.unscanned).toEqual([]);
