 ...
 *  file lands its tenant on s2/prod, so a single-stage stand-in is all TenantRegistry needs to
...
function lifecyclePorts(registry: TenantRegistry): TenantLifecyclePorts {
  return { registry, resolver: tenantResolver(), catalogRepoUrl: DEPLOY_URL, argoWatchTimeoutMs: 1000, resolveUnitApex: async () => "example.com", dns: new FakeDnsProvider() };
}
...
function onboardPorts(registry: TenantRegistry): TenantOnboardPorts {
  return {
...
/** The tenant routes with the purge family wired. The registry rides with the family exactly as it does
 *  in production (buildTenantOnboarding constructs BOTH or neither), so `enabled: false` also means "no
 *  pointer registry" — which is what makes the scan route's degrade branch reachable. tenant-suspend is
 *  wired only so a test has a run of ANOTHER kind to point the purge-target route at. */
async function makeTenant(enabled: boolean): Promise<{ app: Hono<AppEnv>; executor: Executor; cookie: string; registry: TenantRegistry; repo: FakePlatformRepo }> {
  const store = new CredentialStore({ db: db.db, logger });
  const bus = new RunEventBus();
  const repo = new FakePlatformRepo(); // exposed so a test can plant a pointer the registry would never write
  const reg = new TenantRegistry(repo, CLUSTER_STAGE);
  const defs = enabled
  ...
  const app = createApp({
    ...
    registerProtected: (a) => registerTenantRoutes(a, { executor, db: db.db, onboardingEnabled: enabled, ...(enabled ? { registry: reg } : {}) }),
  });
  const cookie = await session.mint({ sub: "op_test", groups: ["admins"], via: "oidc" });
  return { app, executor, cookie, registry: reg, repo };
}

/** A live tenant.yaml in the fake catalog — the ONE thing that makes a tenant an orphan when no
 *  tenants row accompanies it. Committed through the real registry, so the scan reads exactly the bytes
 *  a create-tenant would have written. */
async function seedPointer(registry: TenantRegistry, guid: string, subdomain: string, over: Partial<TenantRegistration & { stage: Stage }> = {}): Promise<void> {
  ...
  await registry.commitTenant({ stage, guid, registration, runId: "run_seed" });
}
...
describe("GET /api/tenants/orphans (the pointer scan)", () => {
  it("lists a live pointer with no inventory row, resolved to its cluster row", async () => {
    seedCluster();
    seedSlaveCluster();
    const { app, cookie, registry } = await makeTenant(true);
    await seedPointer(registry, ORPHAN_GUID, "ghost.example");
  ...
  it("a pointer whose tenant IS recorded is not an orphan", async () => {
    seedCluster();
    seedSlaveCluster();
    db.db.insert(tenants).values({ id: "tnt_1", clusterId: "cls_2", guid: ORPHAN_GUID, subdomain: "ghost.example", stage: "prod", members: ["auth", "jobs", "report"], identityProvider: "auth", provenance: "controller", status: "active" }).run();
    const { app, cookie, registry } = await makeTenant(true);
    await seedPointer(registry, ORPHAN_GUID, "ghost.example");
  ...
  it("carries every pointer it could NOT read to the browser — an empty list may never mean 'unchecked'", async () => {
    ...
    seedCluster();
    seedSlaveCluster();
    const { app, cookie, registry, repo } = await makeTenant(true);
    await seedPointer(registry, ORPHAN_GUID, "ghost.example");
  ...
  it("FAILS SOFT — an unreadable catalog answers 200 carrying the failure, never a bare empty list", async () => {
    ...
    const { app, cookie, registry } = await makeTenant(true);
    registry.listTenantPointers = () => Promise.reject(new Error("catalog unreachable"));
...
describe("GET /api/tenants/runs/:runId/tenant-state (the run's tenant, as inventory has it)", () => {
  ...
  it("with an ACTIVE row: LIVE — the failed-at-`activate` run, which must NOT be offered a purge", async () => {
    ...
    db.db.insert(tenants).values({ id: "tnt_live", clusterId: "cls_2", guid, subdomain: "acme.example", stage: "prod", members: ["auth", "jobs", "report"], identityProvider: "auth", provenance: "controller", status: "active" }).run();
  ...
  it("with a PROVISIONING row: UNFINISHED — the one row state a purge is the remedy for", async () => {
    ...
    const { app, executor, cookie } = await makeTenant(true);
    const { runId, guid } = await planned(app, executor, cookie);
    db.db.insert(tenants).values({ id: "tnt_half", clusterId: "cls_2", guid, subdomain: "acme.example", stage: "prod", members: ["auth", "jobs", "report"], identityProvider: "auth", provenance: "controller", status: "provisioning" }).run();
    const body = (await (await app.request(`/api/tenants/runs/${runId}/tenant-state`, authed(cookie))).json()) as { state: string; row: Record<string, unknown> };
  ...
  it("with an OFFBOARDED row: OFFBOARDED — the tenant is already gone, so nothing is offered", async () => {
    ...
    const { app, executor, cookie } = await makeTenant(true);
    const { runId, guid } = await planned(app, executor, cookie);
    db.db.insert(tenants).values({ id: "tnt_gone", clusterId: "cls_2", guid, subdomain: "acme.example", stage: "prod", members: ["auth", "jobs", "report"], identityProvider: "auth", provenance: "controller", status: "offboarded" }).run();
  ...
  it("404 for an unknown run, 400 for a run of another kind", async () => {
    seedCluster();
    db.db.insert(tenants).values({ id: "tnt_1", clusterId: "cls_1", guid: ORPHAN_GUID, subdomain: "acme.example", stage: "prod", members: ["auth", "jobs", "report"], identityProvider: "auth", provenance: "controller", status: "active" }).run();
...
describe("POST /api/tenants/purge (the force-offboard trigger)", () => {
  ...
  it("plans against a LIVE pointer's fan-out when the orphan has one", async () => {
    seedCluster();
    seedSlaveCluster();
    const { app, executor, cookie, registry } = await makeTenant(true);
    await seedPointer(registry, ORPHAN_GUID, "ghost.example");
