...
//
// WHY THE MANAGER MAKES THEM. Everything else a tenant needs is claimed by the member chart that
...
// a per-tenant writer, and the Manager is already the minter of exactly these kinds for a consumer.
//
 ...
 *  Manager's write-only property intact — read-before-write would need a read grant on every
 ...
 *  where the token is NOT declared). So the Manager asks for what was made rather than holding a
