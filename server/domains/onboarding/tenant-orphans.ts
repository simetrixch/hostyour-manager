...
//
// Boundary: domain layer — inventory reads + the TenantRegistry + tenant-values' cluster resolution,
...
// reads and the registry's git reads. The scan CLONES catalog, which is why its route is
 ...
 *  inside the registry scan, scanTenantDir) and they do not vanish either — they come back in `skipped`,
...
export async function scanOrphanTenants(deps: { db: Db; registry: TenantRegistry }): Promise<OrphanScan> {
  const { db, registry } = deps;
  const found: OrphanTenantView[] = [];
  const skipped: SkippedTenantPointerView[] = [];
  for (const stage of STAGE) {
    // The inventory side of the diff: every tenant this controller BELIEVES is deployed at this stage.
    ...
    const scan = await registry.listTenantPointers(stage);
