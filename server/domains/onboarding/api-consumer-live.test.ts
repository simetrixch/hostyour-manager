...
// of what the Controller BELIEVES; this reads the FACTS (a cluster smoke + the ArgoCD status) and
// compares what the Application PINS against what the cluster RUNS. The Controller's recorded SHA is no
...
/** A Registry over a fake platform repo whose install branch states an apex that is NOT the cluster's
 *  domain — the shape every real cluster has, since install.sh defaults `unit-apex` to the cluster
 *  FQDN minus its first label. Without a registry the route cannot read a chain at all, which is the
 *  null case every other test in this file exercises. */
function apexRegistry(unitApex: string): Registry {
...
async function makeConsumerLive(resolver?: FakeClusterKubeResolver, registry?: Registry): Promise<{ app: Hono<AppEnv>; cookie: string }> {
  const store = new CredentialStore({ db: db.db, logger });
  const bus = new RunEventBus();
  // The live route is a pure READ — no run defs are needed; the bare registry satisfies the executor dep.
  const executor = new Executor({ db: db.db, creds: store, bus, logger, registry: buildRegistry({ db: db.db }), sshFactory: noSsh, actor: () => "op_system" });
  const session = new SessionCodec(db.db, config);
  const app = createApp({
    config, logger, getReadiness: () => ({ ok: true, checks: [] }), session,
    registerAuth: () => undefined,
    registerProtected: (a) => registerConsumerRoutes(a, { executor, db: db.db, store, onboardingEnabled: true, ...(resolver ? { resolver } : {}), ...(registry ? { registry } : {}) }),
  });
...
describe("the consumer's public address on the live payload", () => {
  it("is <name>.<unitApex> off the cluster's OWN values chain — not <name>.<clusterDomain>", async () => {
    seedConsumer();
    const { app, cookie } = await makeConsumerLive(liveResolver(SMOKE_OK, threeSourceApp({ targets: SHA, synced: SHA })), apexRegistry("example.com"));
  ...
  it("follows the apex the chain states, so a cluster that IS its own apex composes that instead", async () => {
    seedConsumer();
    const { app, cookie } = await makeConsumerLive(liveResolver(SMOKE_OK, threeSourceApp({ targets: SHA, synced: SHA })), apexRegistry("s1.example"));
...
describe("consumer live reconciliation (GET /api/consumers/:appId/live)", () => {
  ...
  // was advanced through GitOps (a commit on the install branch, not a Controller run), so the
  // Application targets DEPLOYED and ArgoCD converged onto it, while the Controller-side record only a
  ...
  // instead of promoting a Controller-side guess to a pin and calling the pair drift. The defect itself
  ...
  for (const status of ["suspended", "offboarded"] as const) {
    it(`a ${status} consumer's ABSENT Application is the intended outcome — never 'drift'`, async () => {
      ...
      // still REPORTED — it is the last thing a Controller run wrote — but it is not promoted to a pin,
