...
function entry(cluster = "s1"): TenantRegistration {
  return {
    members: testMembers([{ name: "erp", seedReference: false, seedDemo: false, selections: {} }]), identityProvider: "auth", routing: "host", ownDomain: "", ownDomainRedirects: [], approvedTags: {},
...
describe("tenant-purge execution", () => {
  ...
  it("SAYS it when no seeder is wired — the one case where the crypto survives a purge", async () => {
    // The seeder is what WROTE the entry, so a Manager without Vault has nothing it could take
