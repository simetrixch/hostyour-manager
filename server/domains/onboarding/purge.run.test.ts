...
function ports(reg: Registry, over: Partial<PurgePorts> & FakeKube = {}): PurgePorts {
...
describe("purge run definition", () => {
  it("plans with cluster targetKind, the ordered steps, and git-branch + master-kube locks — with NO app row", async () => {
    ...
    const def = makePurgeDef(ports(new Registry(new FakePlatformRepo(), prodClusterStage)));
  ...
  it("mutating def starts with attest-target under empty params (the armed check does def.steps({}))", () => {
    ...
    expect(makePurgeDef(ports(new Registry(new FakePlatformRepo(), prodClusterStage))).steps({} as PurgeParams)[0]?.name).toBe("attest-target");
  });
  ...
  it("does NOT require an inventory row — the whole run is a no-op-safe teardown even when nothing exists", async () => {
    ...
    await expect(runAll(ports(new Registry(new FakePlatformRepo(), prodClusterStage), { cluster }), logs)).resolves.toBeUndefined();
  ...
  it("remove-registration is idempotent when the registration is already absent (orphan died before write-registration)", async () => {
    seedCluster();
    const reg = new Registry(new FakePlatformRepo(), prodClusterStage); // no registration committed
  ...
  it("watch-removal is FAIL-SOFT: it does NOT throw when the app never prunes (the core difference from offboard)", async () => {
    ...
    const step = makePurgeDef(ports(new Registry(new FakePlatformRepo(), prodClusterStage), { argo })).steps(PARAMS).find((s) => s.name === "watch-removal")!;
    const logs: string[] = [];
  ...
  it("delete-appproject reaps the pre-seeded isolation AppProject and is idempotent when absent", async () => {
    ...
    const projects = new FakeMasterProjectWriter();
    await projects.applyAppProject("argocd", renderConsumerAppProject({ name: "acme", namespace: "acme", repoURL: "https://github.com/x/acme.git", platformRepoURL: "https://github.com/x/hostyour-cloud.git", argoNamespace: "argocd" }));
    const del = makePurgeDef(ports(new Registry(new FakePlatformRepo(), prodClusterStage), { projects })).steps(PARAMS).find((s) => s.name === "delete-appproject")!;
  ...
  it("delete-namespace deletes the target-cluster namespace named for the consumer, and is idempotent", async () => {
    seedCluster();
    const cluster = new FakeClusterReader({ deployState: { domain: "s1.example", stage: "prod", writtenAt: "x", generation: 1 } });
    const step = makePurgeDef(ports(new Registry(new FakePlatformRepo(), prodClusterStage), { cluster })).steps(PARAMS).find((s) => s.name === "delete-namespace")!;
    const logs: string[] = [];
  ...
  it("remove-repo-pat deletes the local build-tier entry by NAME and revokes the sealed credential when a row carries one", async () => {
    ...
    const step = makePurgeDef(ports(new Registry(new FakePlatformRepo(), prodClusterStage), { seeder })).steps(PARAMS).find((s) => s.name === "remove-repo-pat")!;
  ...
  it("remove-dns removes the unit's record and STILL fails the run on a DNS API failure (purge's one fail-closed teardown step)", async () => {
    ...
    const step = makePurgeDef(ports(new Registry(new FakePlatformRepo(), prodClusterStage), { dns })).steps(PARAMS).find((s) => s.name === "remove-dns")!;
    ...
    const step2 = makePurgeDef(ports(new Registry(new FakePlatformRepo(), prodClusterStage), { dns: failingDns })).steps(PARAMS).find((s) => s.name === "remove-dns")!;
  ...
  it("remove-webhook deletes the build webhook by NAME when a row carries the repo URL + PAT", async () => {
    ...
    const step = makePurgeDef(ports(new Registry(new FakePlatformRepo(), prodClusterStage), { github })).steps(PARAMS).find((s) => s.name === "remove-webhook")!;
    const logs: string[] = [];
  ...
  it("remove-release-kit git-rm's the release-kit BY NAME when a row carries the repo URL + PAT", async () => {
    ...
    const step = makePurgeDef(ports(new Registry(new FakePlatformRepo(), prodClusterStage), { consumerRepo })).steps(PARAMS).find((s) => s.name === "remove-release-kit")!;
    const logs: string[] = [];
  ...
  it("remove-app-secrets deletes the consumer-tier entry so a re-onboard cannot inherit the old keys", async () => {
    seedCluster();
    const seeder = new FakeSeeder();
    const step = makePurgeDef(ports(new Registry(new FakePlatformRepo(), prodClusterStage), { seeder })).steps(PARAMS).find((s) => s.name === "remove-app-secrets")!;
    const logs: string[] = [];
  ...
  it("remove-database-secrets metadata-deletes the consumer-tier postgres leaf by NAME (unconditional, 404-tolerant no-op for a non-postgres consumer)", async () => {
    ...
    const seeder = new FakeSeeder();
    const step = makePurgeDef(ports(new Registry(new FakePlatformRepo(), prodClusterStage), { seeder })).steps(PARAMS).find((s) => s.name === "remove-database-secrets")!;
    const logs: string[] = [];
  ...
  it("purging ONE stage of a two-stage unit keeps everything the other stage needs", async () => {
    ...
    const reg = new Registry(platform, async (cluster) => ({ name: cluster, stage: cluster === "s2" ? "dev" : "prod" }));
