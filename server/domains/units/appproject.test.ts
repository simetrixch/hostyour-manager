...
describe("renderTenantAppProject", () => {
  ...
  it("names the project == namespace == <guid>-<member>-<stage> and lives in the per-slave ArgoCD namespace", () => {
    ...
    expect(project.metadata.name).toBe("e2e8ymj86dk8-auth-prod");
  ...
  it("gives every member its OWN project, pairwise different and never the bare guid", () => {
    ...
    expect(names).toEqual([
      "e2e8ymj86dk8-auth-prod",
      "e2e8ymj86dk8-jobs-prod",
      "e2e8ymj86dk8-report-prod",
      "e2e8ymj86dk8-erp-prod",
      "e2e8ymj86dk8-web-prod",
    ]);
  ...
  it("pins a single destination to the tenant's OWN cluster by ArgoCD name + THAT MEMBER's namespace (never server '*')", () => {
    expect(project.spec.destinations).toEqual([{ name: "s1", namespace: "e2e8ymj86dk8-auth-prod" }]);
  ...
  it("fences a member out of its SIBLINGS: its project permits its own namespace and no other", () => {
    ...
    expect(erp.spec.destinations).toEqual([{ name: "s1", namespace: "e2e8ymj86dk8-erp-prod" }]);
    expect(erp.spec.destinations.map((d) => d.namespace)).not.toContain("e2e8ymj86dk8-auth-prod");
  });
