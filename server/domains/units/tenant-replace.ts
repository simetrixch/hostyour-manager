...
//
// Boundary: domain layer (onboarding) — inventory reads + the TenantRegistrations + the pure fan-out
// algebra + tenant-values; no adapters, no IO beyond the db reads and the registrations's git reads.
...
export function ensureSubdomainFreeStep(
  ...
  return {
    ...
    run: async (ctx) => {
      const current = await resolveReplaceTargets({ db: ctx.db, registrations }, p.stage, p.subdomain);
...
export async function resolveReplaceTargets(
  ...
  for (const g of await registrations.subdomainGuids(stage, subdomain)) guids.add(g);
