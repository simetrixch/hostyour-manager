...
/** Only the ports the steps under test reach: the gates (repo/runner/registry), the registration
 *  writer (registry) and the admission-policy apply (resolver). The release-cycle ports stay unwired
...
function ports(over: Partial<OnboardPorts> & { cluster?: FakeClusterReader } = {}): OnboardPorts {
  ...
  return {
    ...
    registry: new Registry(platformRepo(), prodClusterStage),
...
describe("onboard with a manifest-declared fqdn", () => {
  ...
  it("planStream REJECTS a fqdn another unit has attested — G19 through the real registration tree", async () => {
    seedCluster();
    const prt = ports();
    await prt.registry.commitRegistration({
  ...
  it("write-registration ATTESTS the fqdn into the stage file, and apply-admission-policy admits it beside the platform address", async () => {
    ...
    expect((await prt.registry.readRegistration("prod", "acme"))?.entry.fqdn).toBe(FQDN);
