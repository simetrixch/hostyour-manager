// WHAT A UNIT'S BUILD MEASURES BEFORE THE APPROVE (hostyour-manager#208): the read-only twins of the
// build steps, in the shape every probe answers (executor/probe.ts). Each function here is hung on
// the step whose run() will meet what it measures, so the finding table on the approve card reads in
// step order and a refusal names the step that would have failed.
//
...
// readable with the identity, and whether one stands at the build plane already. WHAT IS NOT:
// Vault's write paths (the seeder is write-only), the registry's identities (no port of the build
// reads it) and the release kit's push right — each of those the run proves by doing, and the table
// says nothing about them rather than something invented.
...
export async function probeIdentity(ports: BuildPorts, p: BuildParams, ctx: ProbeCtx): Promise<PreflightCheck[]> {
  ...
  return withIdentity(ctx, p, "consumer-onboard:probe-identity", async (token, viaApp) => {
    if (viaApp) {
      ...
      return [preflightCheck("identity", title, "hard", "pass", "the platform's GitHub App reaches it and minted a token")];
    }
    let scopes;
    try {
      scopes = await ports.github!.readTokenScopes({ owner, repo, token, signal: ctx.signal });
    } catch (err) {
      if (err instanceof WebhookScopeError) return [preflightCheck("identity", title, "hard", "fail", `the PAT is invalid or expired (${err.message})`, `provide a classic PAT with ${requiredConsumerPatScopesSummary()}`)];
      throw err;
    }
    if (!scopes.classic) return [preflightCheck("identity", title, "hard", "fail", "the token is fine-grained, which reports no scopes", `provide a CLASSIC PAT with ${requiredConsumerPatScopesSummary()}`)];
    const missing = missingConsumerPatScopes(scopes.scopes);
    return [missing.length === 0
      ? preflightCheck("identity", title, "hard", "pass", `a classic PAT with ${scopes.scopes.join(", ")}`)
      : preflightCheck("identity", title, "hard", "fail", `the PAT lacks ${missing.join(", ")}`, `provide a classic PAT with ${requiredConsumerPatScopesSummary()}`)];
  });
}
...
export async function probePackages(ports: BuildPorts, p: BuildParams, ctx: ProbeCtx): Promise<PreflightCheck[]> {
  ...
  try {
    ...
    if (scopes.length === 0) return [preflightCheck("packages", "Private npm packages", "soft", "pass", "the repository routes no scope to GitHub Packages — no packages reader needed")];
    ...
    if (!readerId) return [preflightCheck("packages", "Private npm packages", "hard", "fail", packagesReaderMissing(owner, repo, scopes, CONSUMER_WIZARD), `record it in ${CONSUMER_WIZARD}`)];
    ...
    return withToken(reader, async (token) => {
      ...
      for (const scope of scopes) {
        ...
        if (!name) { out.push(preflightCheck(`packages.${scope}`, title, "soft", "pass", "the lockfile names none")); continue; }
        const answer = await ports.github!.readPackage({ scope, name, token, signal: ctx.signal });
        out.push(answer === "readable"
          ? preflightCheck(`packages.${scope}`, title, "hard", "pass", `@${scope}/${name} is readable with the packages reader of ${owner}`)
          : answer === "absent"
            ? preflightCheck(`packages.${scope}`, title, "hard", "warn", `@${scope}/${name} is not published there`)
            : preflightCheck(`packages.${scope}`, title, "hard", "fail", `@${scope}/${name} is not readable with the packages reader of ${owner}`, `record a packages reader of ${scope === owner ? owner : `${owner} that also reads @${scope}`} in ${CONSUMER_WIZARD}`));
      }
      return out;
    });
  } finally {
...
export async function probeWebhook(ports: BuildPorts, p: BuildParams, ctx: ProbeCtx): Promise<PreflightCheck[]> {
  ...
  if (!ports.webhookSecret) return [unmeasuredCheck("webhook", title, "GITHUB_WEBHOOK_SECRET is not configured on this manager")];
  ...
  return withIdentity(ctx, p, "consumer-onboard:probe-webhook", async (token, viaApp) => {
    try {
      ...
      if (refusal) return [preflightCheck("webhook", title, "hard", "fail", `${refusal.reading} (${status})`, refusal.hint)];
      return [preflightCheck("webhook", title, "hard", "fail", `the identity cannot read the hooks (${status})`, "provide a PAT with admin:repo_hook")];
    }
  });
}
