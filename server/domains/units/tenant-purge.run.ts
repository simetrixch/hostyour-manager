...
// registration, which the FIRST teardown step git-rm's. steps() is synchronous and has no db/registrations
  ...
  return {
    ...
    planStream: async (rawParams, ctx) => {
      ...
      const plan: Plan = {
        ...
        locks: tenantLocks(ports.registrations), // the tenant registrations' branch + the one master ArgoCD, like every tenant run
