 ...
 *  `<name>-build` namespace with its EventListener and controller-read grants, the repo PAT at
...
export async function unitStaysRegistered(
  ...
  const elsewhere = (await registry.readUnitStages(unit.name)).filter((standing) => standing !== unit.stage);
...
/** The ports a tenant lifecycle run drives: the TenantRegistry (sole writer of tenants/**, NOT the
 *  consumer Registry), the per-cluster kube resolver (which yields the projectWriter offboard
 *  needs to delete the isolation AppProject), plus the catalog repo URL the renderer pins
 *  sourceRepos to. The only shape difference from LifecyclePorts is registry + repoURL. */
export interface TenantLifecyclePorts {
   ...
   *  — the tenant registry's own repo is catalog, so the apex arrives as a resolver, the same
