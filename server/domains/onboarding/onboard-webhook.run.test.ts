...
const emptyRegistry = new Registry(new FakePlatformRepo(), () => Promise.resolve({ name: "s1", stage: "prod" }));
 ...
 *  reaches the create needs an address; so is the registry, which the abort cleanup reads. */
function step(ports: Record<string, unknown>): Step {
  return setupWebhookStep({ resolveBuildPlaneFqdn, registry: emptyRegistry, ...ports } as unknown as OnboardPorts, params());
}
...
describe("onboard setup-webhook step", () => {
  ...
  it("the abort cleanup removes the hook the step created — no orphan on a not-fully-onboarded consumer", async () => {
    ...
    const github = new FakeGitHubConsumer();
    const prt = { github, webhookSecret: "hmac_test", webhookSubdomain: "build", resolveBuildPlaneFqdn, registry: emptyRegistry } as unknown as OnboardPorts;
  ...
  it("the abort cleanup KEEPS the hook while the unit stands at another stage — the repo carries only one", async () => {
    ...
    const registry = new Registry(new FakePlatformRepo(), () => Promise.resolve({ name: "s2", stage: "dev" }));
    ...
    const github = new FakeGitHubConsumer();
    const logs: string[] = [];
    const prt = { github, webhookSecret: "hmac_test", webhookSubdomain: "build", resolveBuildPlaneFqdn, registry } as unknown as OnboardPorts;
