...
// The Manager writes it imperatively with the unit's ONE PAT: the PAT lives in the sealed store
// and the local build Vault, neither of which the target's ESO reads, so no chart-rendered
// ExternalSecret could materialize it — the Manager is the only writer that holds the value.
//
  ...
  return {
    ...
    metadata: {
      ...
      labels: {
        ...
        // The Manager ownership label — the writer refuses to touch a Secret without it.
