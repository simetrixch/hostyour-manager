...
async function make(registrations?: Registrations): Promise<{ app: Hono<AppEnv>; cookie: string }> {
...
describe("what the three sizes cost ONE unit", () => {
  ...
  it("says so rather than guessing when the registration cannot be read", async () => {
    // No registrations ⇒ consumer onboarding is unwired. Answering with the base rows AND saying they are
