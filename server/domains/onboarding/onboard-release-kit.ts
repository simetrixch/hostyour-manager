...
export function injectReleaseKitStep(ports: OnboardPorts, p: OnboardParams): Step {
  return {
    ...
    run: async (ctx) => {
      ...
      // needs the release client), so an unwired consumer-repo writer is a controller misconfiguration,
      // never a silent skip.
      if (!ports.consumerRepo) {
        throw errValidation(`onboard "${p.consumerName}" requires the consumer-repo git writer but none is wired on this controller — refusing to onboard a consumer whose repo cannot mint a release (no release kit → no release build)`);
      }
...
export async function removeReleaseKit(
  ...
  if (!opts.consumerRepo) {
    ctx.log("meta", `release-kit removal skipped for ${opts.consumerName} — no consumer-repo git writer is wired on this controller`);
    return;
  }
