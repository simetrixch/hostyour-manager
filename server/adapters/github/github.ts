// GitHub REST adapter (branches feature + reset). ONLY this module speaks the GitHub HTTP API.
// Used by the Branches view (list + compare) and the Reset wizard (delete branches + strip
// pointer files). The token is the Controller-dedicated GITHUB_WRITE_PAT (fine-grained,
// Contents: read+write on THIS repo) — deliberately NOT the slave-copied read-only
// GITOPS_REPO_PAT — injected into the Controller as env (config.github).
...
export interface GitHubClient {
   ...
   *  untouched (commitSha: null) — idempotent. Throws GitHubError when the branch is missing,
...
function assertSaneRef(name: string): void {
  if (name.split("/").some((s) => s === "" || s === "." || s === "..")) {
    throw new GitHubError(`refusing suspicious ref name: ${name}`);
  }
}

/** Build a GitHub client. `fetchImpl` is injectable so tests never hit the network. */
export function createGitHubClient(cfg: GitHubConfig, fetchImpl: FetchLike = fetch): GitHubClient {
  ...
  async function req(path: string, init?: RequestInit, allow: number[] = [404]): Promise<Response> {
    ...
    try {
      ...
      throw new GitHubError(`GitHub request failed (${path}): ${e instanceof Error ? e.message : String(e)}`);
    }
  ...
  async function loadTree(branch: string): Promise<{ headSha: string; baseTree: string; blobs: string[] }> {
    ...
    if (refRes.status === 404) throw new GitHubError(`branch not found: ${branch}`, 404);
    ...
    if (tree.truncated) throw new GitHubError(`tree listing for ${branch} is truncated — refusing a blind delete`);
  ...
  return {
    async listBranches(): Promise<BranchRef[]> {
      ...
      for (let page = 1; page <= 20; page++) {
        // allow: [] — a 404 (repo renamed / PAT lost access) must throw a GitHubError with
      ...
      if (!complete) throw new GitHubError(`branch listing exceeded 2000 (20 pages of 100) — refusing a truncated enumeration`);
    ...
    async compare(base: string, head: string): Promise<BranchComparison> {
      ...
      if (res.status === 404) throw new GitHubError(`branch not found comparing ${base}...${head}`, 404);
