...
function harness(): Harness {
  const registry = new TenantRegistry(seededPlatformRepo(), CLUSTER_STAGE);
...
const registeredApps = async (h: Harness): Promise<string[]> =>
  ((await h.registry.readTenant("prod", GUID))?.entry.apps ?? []).map((a: { name: string }) => a.name);
