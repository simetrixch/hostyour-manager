...
/** A migrations folder applied under a ledger of its own: a compiled plugin's (server/plugin.ts). */
 ...
 *
 * The core's folder runs first, under `__drizzle_migrations`; then each tree in the order given
 * (the caller passes them in dependency order), under `__drizzle_migrations_<name>`. drizzle's
 * migrator gates only on the newest `created_at` in the ledger it is told, so each tree keeps its
 * own history and none can shadow another's.
 *
 ...
 */
export function openDb(file: string, trees: readonly MigrationTree[] = []): DbHandle {
  ...
  for (const tree of trees) migrate(db, { migrationsFolder: tree.migrations, migrationsTable: `__drizzle_migrations_${tree.name}` });
