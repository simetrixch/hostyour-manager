...
// exceptions are the objects the CONTROLLER wrote imperatively, outside any chart, either because the
...
export async function assertNoOrphans(ctx: StepCtx, ports: OrphanScanPorts, unit: AppCluster, verb: string): Promise<void> {
  ...
  const standing = await ports.registry.readUnitStages(unit.name);
  ...
  // so a controller wired without it never wrote the object either and there is nothing of its making
  ...
  if (ports.repoCredential) {
    ...
    unread.push("the ArgoCD repository Secret (no repository-credential writer is wired, so this controller never wrote one)");
  }
  ...
  if (ports.buildRbac) {
    ...
    unread.push("the build grants (no build-RBAC writer is wired, so this controller never wrote any)");
  }
