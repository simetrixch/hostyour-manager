...
describe("create-tenant planStream resolves the registry host", () => {
  it("freezes the registry of a cluster whose build plane is NOT its master — never zot.<master>", async () => {
    seedClusters();
    ...
    const prt = ports({
      resolveClusterValueFiles: async (domain, stage) => {
        ...
        return [{ path: "cluster/profile.yaml", content: "global:\n  endpoints:\n    registry:\n      host: zot.build1.example\n" }];
      },
    });
  ...
  it("rejects loud when the target cluster's chain resolves no registry host", async () => {
    ...
    const prt = ports({ resolveClusterValueFiles: async () => [{ path: "cluster/profile.yaml", content: "global: {}\n" }] });
      ...
      doc("Deployment", {
        raw: { kind: "Deployment", spec: { template: { spec: {
          containers: [
            ...
            { name: "cache", image: "docker.io/library/redis:7" }, // a foreign registry is never ours to check
          ],
