...
import { CATALOG_CHART_BRANCH, type TenantRegistry } from "./tenant-registry.ts";
...
// Controller acts master-locally, so the git/helm/kube clients ride in TenantOnboardPorts.
// Two-repo wiring: registry is the TenantRegistry bound to catalog (a SECOND
// PlatformRepo, distinct workRoot), never the consumer Registry. v1 seeds NO secrets
// (requiredSecrets=[]) — a tenant's charts pull from Vault via ExternalSecret.

/** The Controller-side clients the tenant steps drive (master-local; no SSH). The same port
 *  set backs add-app.run.ts. registry is the TenantRegistry, sole writer of tenants/** on
 *  catalog; helm renders the trusted first-party charts controller-side (no sandbox). */
export interface TenantOnboardPorts {
  ...
  catalogCredentialId?: string; // the controller's first-party catalog read credential
  argoWatchTimeoutMs: number;
  /** The ensure-images gate: probes the target cluster's registry for every image the pinned fan-out pulls,
   ...
   *  so no tenant chart can render them and the Controller is the only writer left — the same reason
   *  the consumer onboard writes its build grants imperatively. */
  buildRbac: BuildRbacWriter;
  /** Every `<unit, build>` pair attested on the registration branch (Registry.listAttestedBuildNames).
  ...
  /** Every unit that holds a registration (Registry.listUnitNames). The subdomain belt refuses a
   ...
   *  HttpActivator instance the consumer onboard uses (wire-onboarding.ts). */
  activator?: Activator;
   ...
   *  build plane resolves THAT registry, never the master's), and the values every member chart is
   ...
   *  seeds through, over the Controller's own write-only Vault identity. Optional only because a
   *  Controller without Vault is a real state (a dev process, the checks); absent ⇒ the seed step fails
...
export const CreateTenantParams = z.object({
  ...
  // The registry images the pinned fan-out pulls (the validated render's container images filtered
...
function createTenantSteps(ports: TenantOnboardPorts, p: CreateTenantParams): Step[] {
  ...
  return [
    ...
    ensureSubdomainFreeStep(ports.registry, ports.consumerNames, p),
    {
      ...
      run: async (ctx) => {
        ...
        //
        // Arm the compensation in the same breath: from the moment the Controller commits to this
        ...
        ctx.log("meta", `tenant ${p.guid} recorded as provisioning on cluster ${p.clusterId} (${rowId}) — the row exists BEFORE any mutation, so a run that dies leaves a tenant the Controller can still name`);
      },
    },
    ...replaceSteps,
    {
      ...
      run: async (ctx) => {
        ...
        //
        // The minted values are never logged, never checkpointed and never returned: the Controller
    ...
    // fan-out pulls from the target cluster's registry must EXIST before the pointer lands, or the
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        const { commit } = await ports.registry.commitTenant({ stage: p.stage, guid: p.guid, registration, runId: ctx.runId });
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        ctx.log("meta", `tenant ${p.guid} recorded as active on cluster ${p.clusterId} with ${p.apps.length} app(s) (provenance controller)`);
      },
    },
...
async function mintFreeGuid(ports: TenantOnboardPorts, stage: Stage): Promise<string> {
  for (let i = 0; i < GUID_MINT_ATTEMPTS; i++) {
    ...
    if ((await ports.registry.scanTenant(stage, candidate)).status === "absent") return candidate;
  }
...
export function makeCreateTenantDef(ports: TenantOnboardPorts): RunDefinition<CreateTenantParams> {
  return {
    ...
    planStream: async (rawParams, ctx) => {
      ...
      const replaces = await resolveReplaceTargets({ db: ctx.db, registry: ports.registry }, rc.stage, req.subdomain);
